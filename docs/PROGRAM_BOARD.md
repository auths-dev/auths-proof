...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | SDK package matrix green; formal aggregation retry, human public-surface review, and field-lab hosted CI open | The [Python package](https://github.com/auths-dev/auths-proof/actions/runs/35660161124), [TypeScript package](https://github.com/auths-dev/auths-proof/actions/runs/35660161144), and [recipes](https://github.com/auths-dev/auths-proof/actions/runs/35660161071) passed on `28f0b6c`; its [CI run](https://github.com/auths-dev/auths-proof/actions/runs/35660161157) passed authoritative, translation, Kani, Lean, live, compliance, dependency, secrets, and preflight computations. Formal evidence attempt 1 was blocked by an artifact-service 403; failed-jobs-only attempt 2 exposed a deterministic artifact-name mismatch, now fixed by a same-run bounded lookup pending hosted verification. The [third-adapter report](product/SELF_HOSTED_THIRD_ADAPTER_TRIAL.md) records an independent zero-context engineering trial, not human adoption. Field-lab commit `1808c12` pins SDK `28f0b6c`; both demos passed local mocked tests on its hosted wheel, while the private repo's hosted jobs did not start due billing. An agent [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) does not substitute for the requested human public-surface review. | Verify the retry-safe formal artifact lookup on the new exact CI revision, then obtain human public-surface review and runnable exact-pinned field-lab hosted jobs. |
...
| 2026-09-21 | Epic 1 field-lab hosted CI | The private `auths-field-lab` repository needs paid Actions minutes or a permitted visibility/billing change; the account refuses to start jobs. | Did not change repository visibility or billing. Exact-pinned Airtable [run 35660692453](https://github.com/auths-dev/auths-field-lab/actions/runs/35660692453), Todoist [run 35660692450](https://github.com/auths-dev/auths-field-lab/actions/runs/35660692450), and Tier 0 [run 35660692469](https://github.com/auths-dev/auths-field-lab/actions/runs/35660692469) each received the payment/spending-limit annotation before execution. Local hosted-wheel mocked tests passed 12/12 and 10/10; Epic 1's hosted done gate remains open. |
| 2026-09-21 | Epic 1 formal evidence aggregation | [CI attempt 1](https://github.com/auths-dev/auths-proof/actions/runs/35660161157/attempts/1) could not download a phase artifact after GitHub intermediary returned 403; failed-jobs-only attempt 2 incorrectly looked for successful phase artifacts under its new attempt number. | Preserve the successful translation/Kani/Lean results; select the latest non-expired bounded artifact from the same run at or before the current attempt, then let the existing closure checks verify its contents. Hosted validation of that workflow correction is pending. |
| 2026-09-21 | Epic 1 human public-surface review | AP-SPEC-051's final metadata gate calls for a human read of the Python/TypeScript API inventories and claim metadata. | The implementation agent published a manual source-level read-through, explicitly not human approval. An owner/reviewer must inspect that artifact and the inventories; no approval is inferred. |
