...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | acceptance review in progress; field-lab hosted CI blocked | Commit `da5f470` passed [CI](https://github.com/auths-dev/auths-proof/actions/runs/35650010786) and both package workflows. `447fa46` prevents an empty conformance report from passing, and `116b32d` clarifies first-run diff. This commit extends the shared mandatory scenario corpus from ten to fourteen with claim/finish failures, restart replay, and post-entry interruption. An independent packaged-wheel trial succeeded on the earlier wheel; the new candidate still needs a clean rerun. Field-lab PR #14 hosted jobs remain blocked before startup by the private-repo payment/spending-limit annotation; local live read-backs do not close that gate. | Verify the corrections in hosted CI, rerun the clean-room trial on the new package, review public surfaces, then obtain runnable field-lab hosted jobs. |
