...
| 060 — evidence-conditioned authority | merged in PR #133 (`1dc213de`); clients merged in PR #134 (`d9c41af6`); §15 on draft PR from `060-sdk-attach` | epic steps 1–4 implemented; §15 (SDK attach, Rust–Lean link) implemented, hosted CI pending; §16–§17 specified (K-of-N observer quorum over operator domains, per-extension attenuation) | #133 full CI green on `91864f96`; Rust, Go, and TypeScript agree on 148 vectors; Lean attenuation theorems registered. §15, local only: packed-wheel and packed-npm expected-before-replacement journeys pass against `auths-gateway-harness` (write authorized; changed value denied and stale observation indeterminate before any lease, in the SDK and at the gateway). 14 Lean refinement theorems link the 11 translated observation predicates to the model, and `cargo xtask formal` passes with 182 claims. Readings are in 0060 §15.4. | Hosted CI green on the §15 PR, then §17 before Epic 5, then §16. Operator: live run. |
...
- **Multi-host gateway.** The gateway's attempt, evidence, and outcome state
  ...
  AP-SPEC-038 §9.1.
- **SDK action validity at a gateway.** `prepare_profile_action` gives an
  action the one-second window `[t, t]`, but the gateway verifies at its own
  clock. So a live SDK-authored action passes only if the gateway verifies it
  within that second (0060 §15.4 reading 7). Fixing this needs an authoring
  choice of bounded action lifetime and a fixture regeneration. Decide it
  with the 061 codec phase or the next gateway live run, whichever comes
  first.
