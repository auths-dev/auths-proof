...
| 0057 Epic 3 — signing under any principal method | `epic-3-signing` (draft PR #129) | step 1 done; step 2 object and envelope vectors written; step 3 library path written (typed actions, method-agnostic signing and verification), CLI trust-source rule pending | Step 1: the [Git signing protocol run](https://github.com/auths-dev/auths-proof/actions/runs/35782280920) passed on hosted Linux and macOS at `b0375a3c`; full CI on that revision deferred behind the formal updater commit `6dfa2c66`. Step 2: 51 object and envelope vectors come from an independent Python generator, and real-Git tests cross-check the parser on SHA-1 and SHA-256 objects; hosted results not checked. The package still has no signing binary and no workspace dependencies. | Step 4: `auths-git-sign`, the delegation commands, software custody, and `--trust-from-ref`. The dependency boundary now allows exactly ten core crates, and `auths-did-keri` is not among them. |
