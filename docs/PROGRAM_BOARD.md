...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | SDK baseline green; formal-evidence retry correction pending exact hosted verification; owner review and literal 0057 field-lab hosted gate open | SDK commit `0266fdc` passed [authoritative/formal CI](https://github.com/auths-dev/auths-proof/actions/runs/35663793124), [Python package](https://github.com/auths-dev/auths-proof/actions/runs/35663793077), [TypeScript package](https://github.com/auths-dev/auths-proof/actions/runs/35663793306), and [recipes](https://github.com/auths-dev/auths-proof/actions/runs/35663793107). The docs-only PR tip `85371b6` passed its SDK packages, recipes, formal translation, Kani, and Lean, but [CI](https://github.com/auths-dev/auths-proof/actions/runs/35668410277) failed formal evidence: attempt 1 hit GitHub's artifact-download 403; failed-jobs attempt 2 exposed an exact-attempt validator conflicting with the selected successful attempt-1 artifact. A bounded fix now binds each selected artifact name to its embedded source attempt and permits only a same-run prior attempt; hosted verification is pending. AP-SPEC-054's five engineering epics have evidence on `0266fdc`; the [third-adapter report](product/SELF_HOSTED_THIRD_ADAPTER_TRIAL.md) is an agent engineering trial, not human adoption. Field-lab `92ac30d` pins that SDK; both demos passed local exact-wheel tests and earlier live read-backs. The owner directed us to stop checking field-lab CI for this handoff, without satisfying AP-SPEC-057's literal gate. The agent [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) is not owner approval. | Verify the retry fix on the new PR tip, then hand off for separate owner review; do not mark Epic 1 done without resolving its distinct hosted-demo gate. |
