...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | hostile single-host boundary passed; isolated live-vendor rerun still open, provider qualification unclaimed | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) is green at `90973d64` in [exact-tip hosted CI](https://github.com/auths-dev/auths-proof/actions/runs/35690791146). Field-lab [commit `fbbeb58`](https://github.com/auths-dev/auths-field-lab/commit/fbbeb58) records a distinct-UID, network-isolated TLS counting-provider run: forged/altered inputs, direct access, competing instance, replay, valid fresh challenge under rotated test trust, race, and crash/restart produced three expected authorized entries and zero unauthorized entries. The ledger limits this to the bound synthetic credential, testkit trust, trusted Docker/host operator, and the single-host store. Earlier Airtable/Todoist live writes used the gateway but same-UID processes. | Repeat both live vendor journeys through `--mode isolated` with credentials entered through the operator channel; do not read `.env` or widen the claim while that remains open. |
