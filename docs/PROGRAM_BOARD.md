...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | SDK package matrix green; formal aggregation retry and field-lab hosted CI open | The [Python package](https://github.com/auths-dev/auths-proof/actions/runs/35660161124), [TypeScript package](https://github.com/auths-dev/auths-proof/actions/runs/35660161144), and [recipes](https://github.com/auths-dev/auths-proof/actions/runs/35660161071) passed on `28f0b6c`; its [CI run](https://github.com/auths-dev/auths-proof/actions/runs/35660161157) passed authoritative, translation, Kani, Lean, live, compliance, dependency, secrets, and preflight computations. Formal evidence attempt 1 was blocked by an artifact-service 403; failed-jobs-only attempt 2 exposed a deterministic artifact-name mismatch, now fixed by a same-run bounded lookup pending hosted verification. The [third-adapter report](product/SELF_HOSTED_THIRD_ADAPTER_TRIAL.md) records an independent zero-context engineering trial, not human adoption. Field-lab commit `1808c12` pins exact SDK `28f0b6c`; both demos passed local mocked tests on its hosted wheel, while the private repo's hosted jobs did not start due billing. | Verify the retry-safe formal artifact lookup on the new exact CI revision, then obtain runnable exact-pinned field-lab hosted jobs. |
