...
| 0057 Epic 4 — publish the evidence | none; [draft PR #124](https://github.com/auths-dev/auths-proof/pull/124) closed without merge on 2026-09-23 | stalled: the vendor corpus is on `main` (`bindings/fixtures/openapi-corpus/`); the 0057 §6 wording corrections, the GTM rewrite, and the unfamiliar-user trial are not checked on `main` | [Manual exact-ref CI](https://github.com/auths-dev/auths-proof/actions/runs/35646269081) ran; authoritative failed only on the dated-review naming allowance missing from its older base. PR #124 held the corpus, wording, and GTM revisions and was closed with no comment; branches `codex/epic-4-evidence` and `epic-4-corpus` remain. | Owner: close Epic 4, or fold its remaining wording and GTM steps into the north-star work. The unfamiliar-user trial is the north star's "Next". |
| 0057 Epic 2 — gateway + hostile proof | merged PR #125 (2026-09-22) | done for the documented single-host development deployment; the multi-host store merged with 038 §9 (PR #143); provider qualification and AP-SPEC-053 adoption claims remain open | [PR #125](https://github.com/auths-dev/auths-proof/pull/125) was green at `90973d64` in [exact-tip hosted CI](https://github.com/auths-dev/auths-proof/actions/runs/35690791146). Field-lab [commit `fbbeb58`](https://github.com/auths-dev/auths-field-lab/commit/fbbeb58) records the distinct-UID hostile run with three expected entries and zero unauthorized entries. Field-lab [commit `afa733b`](https://github.com/auths-dev/auths-field-lab/commit/afa733b) records fresh Airtable and Todoist writes through `--mode isolated`: both preflight isolation probes passed, Airtable returned `observed` with matching read-back, and Todoist returned `response-recorded` followed by independent task observation. The ledger limits this to the bound credentials, testkit trust, trusted Docker/host operator, and the single-host store. | Do not relabel this as provider qualification or production trust; production-trust engineering is the 038 §9 row. |
...
| 060 — evidence-conditioned authority | merged in PR #133 (`1dc213de`); clients merged in PR #134 (`d9c41af6`); §15 merged in [PR #144](https://github.com/auths-dev/auths-proof/pull/144); §17 merged in PR #141 | epic steps 1–4, §15 (SDK attach, Rust–Lean link), and §17 (per-extension attenuation) implemented; §16 (K-of-N observer quorum over operator domains) specified, not started | #133 full CI green on `91864f96`; Rust, Go, and TypeScript agree on 148 vectors; Lean attenuation theorems registered. §15, local only: packed-wheel and packed-npm expected-before-replacement journeys pass against `auths-gateway-harness` (write authorized; changed value denied and stale observation indeterminate before any lease, in the SDK and at the gateway). 14 Lean refinement theorems link the 11 translated observation predicates to the model, and `cargo xtask formal` passes with 182 claims. SDK actions carry a bounded validity window (30 s by default, at most 300 s), so they verify at a later gateway clock. Readings are in 0060 §15.4. | §16 is demand-gated (§3). Operator: live run. |
| 0057 Epic 5 — bounded policy in the path | merged PR #141 (step 1) and PR #149 (step 2) | engineering merged; the parts of 0025 tranches 4–6 listed in 0025 §24.3 stay open | Step 1: handler-declared per-extension attenuation laws, 163 corpus vectors on which Rust, Go, and TypeScript agree, re-qualified kernel translation, and Lean theorems that delegation never widens authority given narrowing laws. Step 2: the `bounded-policy-commitment-v1` extension and link law (registry manifest `36`, 173 vectors in parity); a closed gateway evaluator registry; one argument-ceiling window-count evaluator with insert-once count slots in the attempt store; Lean proofs of fixed-context tightening and decider soundness, recorded as the product-layer premise; and the per-principal hostile suite (`bindings/fixtures/gateway/bounds-hostile.json`), whose refused cases have zero provider entries and zero leases. Both merged to `main`. | None; the open 0025 §24.3 parts are Not now unless the north star needs them. |
...
| 2 | **Stalled: PR #124 closed unmerged (§1).** 0057 Epic 4 — publish the evidence (steps 1, 3, 4 only) | Epic 1 pushed (may run while its CI runs) | corpus for GitHub + two vendor documents under `bindings/fixtures/openapi-corpus/` with rejection walls; every §6 wording row in 0057 corrected at its cited location; GTM doc rewritten per 0057 Epic 4 step 4. Step 2 (unfamiliar-user trial) needs a human or a zero-context agent in a separate task; if neither is available, log in §9 | `epic-4-evidence` |
...
| 2026-09-24 | Owner decision: the approval-quorum action window is configurable with a default of 24 hours, so human approvers are not held to minutes. The default is held only in `auths-approval-quorum`; the single-signer 30 s default and 300 s cap in `auths-author` are unchanged. The window is cut to the earliest approver grant expiry, custody requests stay valid for the whole window, and core `ActionValidityPolicy` is the one place the window arithmetic lives. | `auths-approval-quorum`; `auths-author`; `docs/product/APPROVAL_QUORUM.md` |
...
| 2026-09-24 | Owner directed the north-star hardening to run in parallel, above the WIP limit of two: build the gateway `Idempotency-Key` for recipe writes (#146), reversing the earlier "correct the docs" reading because the journey now sends Stripe refunds through a recipe; the gateway path of #148; and the journey from the npm package. | §0, §1; #146; #148 |
| 2026-09-24 | Owner: review-pass records under `docs/audit/passes/` stay local and are ignored by git; a finding that changes the design becomes a spec under `docs/specs/`. | `docs/audit/README.md`; PR #151 |
...
| 2026-09-23 | AP-SPEC-038 §9.3 second operator | The separation and custody requirements must be exercised by a second operator. | Implemented the checks and tests; did not simulate an operator. |
| 2026-09-23 | AP-SPEC-038 §9.3 observer quorum | A 2-of-3 observer quorum across three operator domains needs AP-SPEC-060 §16, which is demand-gated. | Did not implement §16; the gate item stays open. |
| 2026-09-23 | AP-SPEC-038 §9.3 root ceremony | The candidate evidence needs a reviewed root ceremony record. | Tested a 2-of-3 root through the kernel's `KOfN`; no ceremony was held or recorded. |
| 2026-09-23 | AP-SPEC-038 §9.1 custody kind in trust | A trusted context requiring a custody kind needs provider attestation and a core wire field. | Signers report custody kind; no verifier-side requirement was added. |
| 2026-09-23 | AP-SPEC-038 §9.1 custody in the binaries | `auths-gateway` and `auths-git` have no KMS or PKCS#11 client (Epic 4 work). | Library paths are custody-held and conformance-tested; production gateway install refuses a software observer. |
