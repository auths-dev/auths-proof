...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | acceptance review documented; field-lab hosted CI blocked | Commit `da5f470` passed [CI](https://github.com/auths-dev/auths-proof/actions/runs/35650010786) and both package workflows. The corrections `447fa46`, `116b32d`, `a750983`, `102f648`, and `c0cdba8` tighten conformance, diagnostics, and the bypass claim; AP-SPEC-055 is implemented without provider qualification. The public-surface [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) is committed. This follow-up keeps all fourteen mandatory conformance cases while allowing bounded provider-specific cases, as AP-SPEC-054 §7 requires. The zero-context third-adapter trial succeeded on the earlier wheel; the corrected candidate still needs a clean rerun. Field-lab PR #14 hosted jobs remain blocked before startup by the private-repo payment/spending-limit annotation. | Verify the corrections in hosted CI, rerun and publish the clean-room trial on the new package, then obtain runnable field-lab hosted jobs. |
