...
| 0057 Epic 3 — signing under any principal method | `epic-3-signing` (draft PR #129) | engineering implemented, steps 1–6; not accepted (two human clauses) | Hosted green on Linux and macOS through the Git signing protocol workflow for steps 1–4 (runs 35782280920, and later ones at `31b640b8`, `1f46f0db`, `6598e168`); step 6 [action self-test](https://github.com/auths-dev/auths-proof/actions/runs/35789962132) green. Step 5 acceptance test verifies `did:key`, OIDC-workload, and Sigstore-keyless (local CA and log) under one root; the live OIDC job and full CI on the final tip are recorded in the PR. Open: public-good Sigstore needs a core DER-ECDSA suite; dogfooding this repository needs owner decisions (§9). | Owner: decide root-key custody and branch protection for this repository, and whether the DER-ECDSA suite work starts under the Sigstore adapter's spec. |
...
| 2026-09-22 | Epic 3 step 5 public-good Sigstore | Verifying real public-good Sigstore output needs a DER-encoded ECDSA P-256 signature suite in core. The only P-256 suite accepts 64-byte low-S signatures; Rekor and cosign produce DER. That is a core wire and registry change, owned by the Sigstore adapter's spec, not by Git signing. | Built the Sigstore signer against a client port and proved sign-and-verify with a deterministic local Fulcio CA and Rekor log. Did not add a core suite inside a product epic. |
