...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress from merged main; compiler and single-host claim store, no enforcement claim | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) contains three SDK-generated fixture profiles and a closed recipe compiler. The next commit adds a single-host one-use claim store and state fixtures. [Hosted run 35679168022](https://github.com/auths-dev/auths-proof/actions/runs/35679168022) stopped in preflight on semantic-freeze drift after formal-manifest regeneration; the freeze update is included here. No provider call, isolated deployment, or hostile-application acceptance is claimed yet. | Obtain green hosted compiler/store evidence, then implement operator binding and the separate execution path. |
