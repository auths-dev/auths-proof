...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | in progress; one real exact-revision failure identified | Exact `89b1beb`: Python package, TypeScript package, ten installed recipes, preflights, dependency, secrets, compliance, and live implementation jobs pass. `authoritative implementation` alone rejects the dated independent-review heading as a stale public product name; an exact negative-document allowance is prepared without editing the review. The public-repository checkout needs no secret; field-lab pin/workflows await a green freeze commit. The `9b31882` title is corrected forward in the claim ledger. | Does the exact naming allowance make the next authoritative run green without weakening current-name enforcement? |
...
| 2026-09-21 | Epic 1 authoritative naming | The dated independent competition review intentionally uses an `Auths Proof` heading, which the current-name policy rejects unless exactly allowed as historical or negative documentation. | Added one path-and-token allowance in `release/public-naming.toml`; did not edit the independent review or weaken the global stale-name rule. |
