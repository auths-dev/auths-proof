...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress; live mechanical gateway path, still unqualified | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) contains three SDK-generated recipe fixtures, compiler, one-use claim store, native-verification coordinator, customer-operated service, and Python/TypeScript proof-action clients. Local packaged-wheel runs reached disposable Airtable and Todoist resources; the ledger records same-UID, testkit-trust, and causation limits without publishing private dashboard links. [Exact-tip CI on `d6c67860`](https://github.com/auths-dev/auths-proof/actions/runs/35687671862) passed implementation packages, authoritative checks, gateway isolation, recipe checks, formal translation, Kani, and Lean, but formal evidence aggregation failed because Lean produced a bounded update artifact instead of a phase artifact. The update contained three changed source-closure hashes in the assurance manifest, matched against the generated local closure; the manifest and its semantic-freeze inventory are included in the next commit. Local formatter, targeted gateway Clippy, binding-semantics, architecture, and semantic-freeze checks passed; authority/formal checks remain hosted-only. | Obtain exact-tip hosted green and expand hostile zero-entry evidence; external adoption and production-style trust remain open. |
