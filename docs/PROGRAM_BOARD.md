...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress from merged main; recipe compiler and fixtures first, no enforcement claim | AP-SPEC-053 §3.2.1 and the type map define the first closed source; three SDK-generated fixture profiles and hostile recipes are being implemented. No provider call, isolated deployment, or hostile-application acceptance is claimed yet. | Compile all three recipes in hosted CI, then implement operator binding and the separate execution path. |
