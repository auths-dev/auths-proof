...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress from merged main; compiler and single-host claim store, no enforcement claim | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) contains three SDK-generated recipe fixtures and a closed compiler; `6ce03768` adds a single-host one-use claim store. [Hosted run 35679817628](https://github.com/auths-dev/auths-proof/actions/runs/35679817628) next rejected an undeclared compliance role; a forward metadata correction is in progress. No provider call, isolated deployment, or hostile-application acceptance is claimed. | Obtain green hosted compiler/store evidence, then implement operator binding and the separate execution path. |
