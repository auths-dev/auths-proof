...
| 0057 Epic 3 — signing under any principal method | `epic-3-signing` (draft PR #129) | step 1 done; step 2 object and envelope vectors written, action vectors moved to step 3 | Step 1: the [Git signing protocol run](https://github.com/auths-dev/auths-proof/actions/runs/35782280920) passed on hosted Linux and macOS at `b0375a3c`; full CI on that revision deferred behind the formal updater commit `6dfa2c66`. Step 2: 51 object and envelope vectors come from an independent Python generator, and real-Git tests cross-check the parser on SHA-1 and SHA-256 objects; hosted results not checked. The package still has no signing binary and no workspace dependencies. | Start step 3: action encoding and vectors, then the `did:key` verifier. That needs the first explicit core dependencies in the `git-signing` boundary. |
