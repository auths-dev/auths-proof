...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | in progress; bounded semantic-freeze update pending CI | Exact `89b1beb`: Python package, TypeScript package, ten installed recipes, preflights, dependency, secrets, compliance, formal translation, and live implementation jobs pass. `authoritative implementation` alone rejected the dated independent-review heading. Head `ca87c5b` applied the exact negative-document allowance, then repository preflight correctly required `auths.release.public-surface` and the inventory freeze to advance from 259 to 260. The public-repository checkout needs no secret; field-lab pin/workflows await a green freeze commit. The `9b31882` title is corrected forward in the claim ledger. | Does the versioned naming allowance make the next repository preflight and authoritative run green without weakening current-name enforcement? |
...
| 2026-09-21 | Epic 1 authoritative naming | The dated independent competition review intentionally uses an `Auths Proof` heading, which the current-name policy rejects unless exactly allowed as historical or negative documentation; versioning that allowance then caused the expected semantic-freeze drift. | Added one path-and-token allowance in `release/public-naming.toml`, advanced only `auths.release.public-surface` and the inventory freeze from 259 to 260, and regenerated the inventory; did not edit the independent review or weaken the global stale-name rule. |
