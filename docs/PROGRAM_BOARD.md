...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress; live mechanical gateway path, still unqualified | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) contains three SDK-generated recipe fixtures, compiler, one-use claim store, native-verification coordinator, customer-operated service, and Python/TypeScript proof-action clients. Local packaged-wheel runs reached disposable Airtable and Todoist resources; the ledger records same-UID, testkit-trust, and causation limits without publishing private dashboard links. On `35cef292`, Python and TypeScript package checks, recipe checks, distinct-UID/restart/competing-service isolation, and repository preflight passed. [Authoritative CI](https://github.com/auths-dev/auths-proof/actions/runs/35686650243) found three TypeScript gateway transport-framing calls without exact binding-semantics declarations; the next commit gives only that unsigned socket envelope and length prefix permanent, written allowances and advances the release-metadata semantic identity. Local binding-semantics, architecture, semantic-freeze, formatter, and targeted gateway Clippy checks passed; authority/formal checks remain hosted-only. | Obtain exact-tip hosted green and expand hostile zero-entry evidence; external adoption and production-style trust remain open. |
