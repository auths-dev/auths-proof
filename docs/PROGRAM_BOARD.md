...
| 060 — evidence-conditioned authority | merged in PR #133 (`1dc213de`); clients merged in PR #134 (`d9c41af6`) | epic steps 1–4 implemented; §15–§17 amendments specified (SDK attach, Rust–Lean link, K-of-N observer quorum over operator domains, per-extension attenuation) | #133 full CI green on `91864f96`; Rust, Go, and TypeScript agree on 148 vectors; Lean attenuation theorems registered. | §17 ships inside Epic 5 (`epic-5-policy`); §15 is queued after the approval-quorum item; §16 is demand-gated (§3). Operator: live run. |
| 0057 Epic 5 — bounded policy in the path | `epic-5-policy` (step 1) | in progress: step 1 (0060 §17) | Per-extension attenuation: handler-declared laws for `exact-marker-v1` and `observation-requirement-v1`, registry manifest `35`, 17 new corpus vectors (163 in all) on which Rust, Go, and TypeScript agree, re-qualified Aeneas translation of the kernel predicates, and Lean theorems that delegation never widens authority given narrowing laws. Local checks only; hosted CI pending on the draft PR. | Review step 1, then AP-SPEC-025 §24 (`bounded-policy-commitment-v1`) on `epic-5-bounds`. |
