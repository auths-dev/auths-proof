...
| 2026-09-23 | PROVISIONAL, on coordinator direction for 0060 §15: an SDK-prepared action is valid for `validity_seconds` after its evaluation time, capped at 300 s by the native authoring path and 30 s by default in the Python and TypeScript SDKs, so live SDK actions verify at a real gateway clock. The challenge and the gateway's exactly-once claim stay the replay defence. | 0060 §15.4 reading 7 |
