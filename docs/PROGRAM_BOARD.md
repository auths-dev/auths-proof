...
| AP-SPEC-056 Epics 1–2 — OpenAPI derivation | branch `epic-6-openapi-derivation` (local commits, not pushed) | Epics 1–2 implemented; Epic 3 (a MAY) not started, reasons in 0056 §8.3 | Local only: `cargo test -p auths-openapi-derive` (74-case corpus, with the pinned vendor documents present), the Python suite on a built extension, TypeScript unit, integration, and package tests on a local WASM build, a clean CPython 3.9 wheel consumer, and `auths gateway recipe check` on derived recipes. No hosted CI result yet; vendor cases do not run in hosted CI. | Push and run hosted CI, update the architecture and semantic-freeze snapshots, and review the PROVISIONAL readings in §4. |
