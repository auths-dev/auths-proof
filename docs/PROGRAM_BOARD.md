...
| 0057 Epic 2 — gateway + hostile proof | `codex/developer-profile-gateway` | in progress; compiler, single-host claim store, and native-verification coordinator; no isolated service yet | [Draft PR #125](https://github.com/auths-dev/auths-proof/pull/125) contains three SDK-generated recipe fixtures, compiler, one-use claim store, typed connection descriptor, and a coordinator that separates verifier, claim, transport, response, and observation stages. [Hosted run 35680174926](https://github.com/auths-dev/auths-proof/actions/runs/35680174926) found semantic-freeze drift from the metadata correction; the next commit refreshes the versioned inventory. No provider call, isolated deployment, or hostile-application acceptance is claimed. | Build the separate application/admin service, test the actual credential boundary, then exercise live disposable resources. |
