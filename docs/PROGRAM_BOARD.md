...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | acceptance review documented; final hosted CI and field-lab hosted CI open | Commit `da5f470` passed [CI](https://github.com/auths-dev/auths-proof/actions/runs/35650010786) and both package workflows. The acceptance corrections through `f5992ba` tighten conformance, diagnostics, and the bypass claim; AP-SPEC-055 is implemented without provider qualification. The public-surface [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) is committed. A fresh clean-room third-adapter rerun on the hosted `f5992ba` wheel passed all fourteen mandatory cases and an exact local HTTP write/read-back; its redacted report is pending publication. [Installed Python typing](https://github.com/auths-dev/auths-proof/actions/runs/35658997720/job/106530642333) found a nested generic class reusing outer `TypeVar`s, now moved to module scope pending hosted verification. A TypeScript packed job received a GitHub artifact-service 403 before tests began. Field-lab PR #14 hosted jobs remain blocked before startup by the private-repo payment/spending-limit annotation. | Publish the clean-room report, verify the typing correction and final SDK revision in hosted CI, then obtain runnable exact-pinned field-lab hosted jobs. |
