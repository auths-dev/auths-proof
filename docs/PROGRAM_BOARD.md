...
| 0057 Epic 3 — signing under any principal method | `epic-3-signing` | in progress; step 1 (envelope and Git program protocol) implemented, hosted evidence pending | [AP-SPEC-058](specs/0058-git-object-signing-under-any-principal-method.md) merged; `auths-git-signing` holds the envelope, the closed program-argument parser, and real-Git tests. The package has no signing binary, so nothing can yet report a good signature. The `git-signing` dependency boundary forbids every workspace dependency until one is explicitly allowed, which keeps `auths-did-keri` out. | Once the `git-signing.yml` Linux and macOS runs are green, start step 2 (fixtures first). |
