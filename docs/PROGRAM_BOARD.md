...
| 0057 Epic 3 — signing under any principal method | `epic-3-signing` (draft PR #129) | steps 1–4 written: envelope, object vectors, method-agnostic signing and verification, `auths-git` / `auths-git-sign` with custody, delegation, revocation, and the trust-source rule | Step 1 [Git signing protocol run](https://github.com/auths-dev/auths-proof/actions/runs/35782280920) passed on hosted Linux and macOS; step 2 passed the same workflow at `65eb4423`. Steps 3–4: local package tests, clippy, and the real-Git end-to-end journey passed on macOS; hosted results not checked. Revocation became a root-signed Auths proof because kernel status must be proof-bound (spec §3.5). | Step 5: Sigstore keyless and OIDC workload signers and one verifier range accepting three methods. |
