...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | acceptance review documented; field-lab hosted CI blocked | Commit `da5f470` passed [CI](https://github.com/auths-dev/auths-proof/actions/runs/35650010786) and both package workflows. The acceptance corrections through `f5992ba` tighten conformance, diagnostics, and the bypass claim; AP-SPEC-055 is implemented without provider qualification. The public-surface [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) is committed. This commit updates 0051/0052/0054 tracking to distinguish landed mechanisms from open final gates. The zero-context third-adapter trial succeeded on the earlier wheel; the corrected candidate still needs a clean rerun. Field-lab PR #14 hosted jobs remain blocked before startup by the private-repo payment/spending-limit annotation. | Verify the corrections in hosted CI, rerun and publish the clean-room trial on the new package, then obtain runnable field-lab hosted jobs. |
