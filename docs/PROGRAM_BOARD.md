...
| --- | --- | --- | --- | --- |
| 0057 Epic 1 — freeze on evidence | codex session on `codex/self-hosted-developer-profiles` | exact SDK CI green; independent owner review and the literal 0057 field-lab hosted gate open | PR #123 SDK commit `0266fdc` passed [authoritative/formal CI](https://github.com/auths-dev/auths-proof/actions/runs/35663793124), [Python package](https://github.com/auths-dev/auths-proof/actions/runs/35663793077), [TypeScript package](https://github.com/auths-dev/auths-proof/actions/runs/35663793306), and [recipes](https://github.com/auths-dev/auths-proof/actions/runs/35663793107). The [third-adapter report](product/SELF_HOSTED_THIRD_ADAPTER_TRIAL.md) records an independent zero-context engineering trial, not human adoption. Field-lab commit `92ac30d` pins that SDK revision; both demos passed local mocked tests on its hosted wheel and earlier live read-backs. Field-lab hosted jobs did not start because of billing, and the owner directed us to stop checking them for PR #123's engineering handoff; this does not silently satisfy AP-SPEC-057's literal acceptance. The agent [read-through](product/SELF_HOSTED_ACCEPTANCE_REVIEW.md) does not substitute for owner review. | Hand off PR #123's engineering result; await separate fresh-session review and an explicit resolution of AP-SPEC-057's hosted-demo gate before marking Epic 1 done. |
