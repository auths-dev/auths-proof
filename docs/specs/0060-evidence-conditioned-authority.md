...
| 7 | Timing of an SDK-authored action at a gateway | `prepare_profile_action` takes an explicit `validity_seconds` in `1..=300`, and anything outside that range is the typed error `ActionValidity`. The action is valid on `[evaluation_time, evaluation_time + validity_seconds]`, so the gateway, which verifies at its own clock (§14 reading 7), accepts it anywhere inside the window. The Python and TypeScript SDKs default to 30 seconds, which can be overridden within the same bound. The window never widens authority: every grant in the chain must contain it, and the challenge and the gateway's exactly-once logical-operation claim remain the replay defence. `sdk_action_window_admits_a_later_gateway_clock_and_replay_stays_refused` shows the effect with the gateway clock 5 s after the SDK's evaluation time: a 30 s window is authorized, a replay inside it is refused with no second write, and a 1 s window is `action-outside-validity` before any lease. |
