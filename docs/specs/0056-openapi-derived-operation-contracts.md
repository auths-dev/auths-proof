...
| `type: integer` with `minimum` and `maximum` inside the safe range | `integer`; a fractional bound rounds inward (lower bound up, upper bound down), an exclusive bound excludes its own value, the tighter of an inclusive and an exclusive bound wins, and a bound outside ±(2^53 − 1) is rejected |
...
| Overrides | Every override must apply to a construct, or it fails with `contract.derive.unused-override`. That code is reported only when nothing else is rejected. `--omit` on a required property is `contract.derive.invalid-override`. `--max-bytes` may only narrow: `N ≤ min(maxLength, 4096)`, since every character is at least one byte and a larger byte bound would admit more ASCII characters than `maxLength`. `--range` may only narrow a bound the document states. `--literal` takes `true`, `false`, a safe integer, or a JSON string of at most 1024 bytes. The value passes the same keyword check as a derived argument: unsupported keywords such as `multipleOf` are rejected, a malformed bound such as `maxLength: 2.5` is an error, `format: byte`/`binary` is rejected, and the value must be a member of any `enum` and within any bound. This holds for body properties and path parameters alike. `--pick` selects exactly one scalar alternative. Override names are shared by parameters and body properties, and a name that reaches more than one construct (a path parameter `id` and a body property `id`) fails with `contract.derive.ambiguous-override`. No spelling separates them, so such an operation is derived by hand. |
| Security | `oauth2` maps to bearer, reported as "acquisition is application-owned". `openIdConnect`, `mutualTLS`, `http` schemes other than bearer, `apiKey` in a query or cookie or in a reserved header such as `Authorization`, and a requirement combining schemes are rejected with `contract.derive.credential-scheme-out-of-scope`. An operation with no `security` key at either level declares nothing. It fails with `contract.derive.missing-security` and needs `--security-scheme`. An explicit `security: []`, or a list whose only entries are `{}`, declares that the operation needs no credential. It fails with `contract.derive.uncredentialed-operation`, has no override, and `--security-scheme` cannot add one, because the gateway sends only credentialed requests. `--security` selects a single-scheme requirement by scheme name. |
...
| Document constructs | Only local `#/` references are followed, and a reference containing `%` is rejected. A referenced path item is rejected, because it hides operations from selection. A keyword other than `description` or `summary` beside `$ref` is rejected. Keywords outside the annotation and mapped sets are rejected with their pointer. A `$ref` string or selected path template longer than 1024 bytes is rejected. Slice measurement renders a pointer only when it reports a diagnostic, and it stops at 256 levels of inlined nesting. A diagnostic pointer longer than 2048 bytes is cut and ends in `...`. The slice measurement reproduces the published corpus figures exactly: 45, 2, and 15 resolutions, and 70 346, 9 455, and 8 960 bytes. |
...
| Re-derivation and `check` | `derive` writes only into an empty directory or one that already holds `derivation.json`. It refuses `profile.toml` or `recipe.json` without `derivation.json` (`contract.derive.directory-not-derived`). It refuses a changed derivation unless `--version` exceeds the recorded version (`contract.derive.version-required`). An unchanged re-derivation writes nothing, and its report says `wrote: nothing`. `derive` refuses any directory where `profile.toml` or `recipe.json` no longer matches `derivation.json`, even with a `--version` bump (`contract.derive.derived-edited`), so a hand edit is never overwritten silently. Both CLIs read `derivation.json` through one native reader. It accepts bounded, duplicate-free UTF-8 JSON with the provenance schema, an integer `profile.version` from 1 to 9999 (`1.0` is rejected), and two lowercase hex digests. `profile check` reports `<file>: derived file edited by hand` when a digest differs; its JSON code is `profile.contract.derived-edited`. `profile diff` prints the same line for each edited file and returns `derived_edits`. Deleting `derivation.json` makes the files hand-owned. |
| CLI-only codes | `contract.derive.document-unreadable` covers a symlink, a non-regular file, and the size bound. `contract.derive.directory-not-derived`, `contract.derive.derived-edited`, and `contract.derive.version-required` are the directory, edit, and version rules above. The mapper's codes are `invalid-request`, `document-invalid`, `document-format`, `unsupported-version`, `operation-not-found`, `duplicate-operation`, `unsupported-method`, `tool-name`, `remote-ref`, `ref-cycle`, `ref-limit`, `slice-limit`, `ambiguous-server`, `unsafe-server`, `ambiguous-security`, `missing-security`, `credential-scheme-out-of-scope`, `query-parameter`, `unsupported-parameter`, `unsupported-body`, `unbounded-string`, `unbounded-integer`, `unsupported-construct`, `scalar-union`, `optional-property`, `open-object`, `compiler-limit`, `enum-variant`, `invalid-name`, `name-collision`, `field-limit`, `invalid-override`, `unused-override`, `ambiguous-override`, `uncredentialed-operation`, `profile-limit` (a `profile.toml` over the generators' 16 KiB), and `recipe-limit` (a recipe over the compiler's 64 KiB), each under `contract.derive.`. |

### 8.2 Corpus and comparison results

The corpus has 87 cases. Derived cases have exact bytes for `profile.toml`,
...
- **Numeric-looking hosts.** `derive` accepts a host such as `0x1.0x2`,
  because its last label is not all decimal digits. The recipe compiler's URL
  parser reads that host as IPv4 and rejects the origin. The derivation fails
  closed, but at `recipe check` rather than at `derive`.
