...
│   └── auths-did-web/
│
├── ../auths-proof-apps/apps/
│   └── downstream CLI and reference services
│
├── examples/
...
### 5.11 `auths-proof-apps/integrations/auths-resolver-did-web`

This crate is intentionally downstream in `auths-proof-apps/integrations`,
not in the proof repository's `adapters/`.
...
A later pure `auths-did-web` adapter can verify the bundled document, document commitment, signature, and purpose. The HTTP resolver is only one way to obtain that evidence.

### 5.12 Downstream CLI (`auths-proof-apps`)
...
The CLI is a downstream reference client and conformance surface, not part of
the proof-kernel workspace.
...
```text
                    ...
                    |                                       |
          +---------+----------+                +-----------+-----------+
          | auths-codec  |                | auths-ports|
          +---------+----------+                +-----------+-----------+
                    ^                                       ^
                    |                                       |
        +-----------+------------+              +-----------+-----------+
        | auths-author     |              | auths-verifier  |
        +-----------+------------+              +-----------+-----------+
                    ^                                       ^
                    +-------------------+-------------------+
                                        |
...
| HTTP | `reqwest` or smaller client | downstream integrations only |
| Async runtime | `tokio` | downstream integrations only |
...
`auths-proof-apps`: profiles, runtime, integrations, receipts, lab tooling,
and reference applications, beginning with an inspectable MCP demo in which a
human or organization root delegates exactly one tool call to an agent.
...
HTTP retrieval is isolated in `auths-proof-apps/integrations`, produces
explicit local trust records, and is absent from the proof repository and
verifier dependency graphs. One
...
- `auths-proof-apps`: downstream profiles and runtime composition beginning
  with one exact MCP `tools/call` profile that executes only after approval.
...
adapter with a versioned ALPN and typed peer observations. `auths-proof-apps`
...
- [ ] `auths-verifier` builds with `no_std + alloc` or the documented minimal WASM profile.
