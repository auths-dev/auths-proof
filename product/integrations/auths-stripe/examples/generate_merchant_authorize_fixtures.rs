...
use auths_stripe::{
    ...
    MerchantAggregateSnapshot, MerchantAuthorizationDecisionReceipt,
    MerchantAuthorizationObservationReceipt, MerchantAuthorizationTransitionReceipt,
    MerchantBudgetWindow, MerchantConnectAccount, MerchantOperation, MerchantPaymentEvidenceInput,
    MerchantPaymentEvidenceV1, MerchantPaymentStore, MerchantProviderProjection,
    PaymentAuthorizeEvaluationContext, PaymentIntentId, PaymentMethodId,
    ...
    StripeMerchantEvaluatorConfigurationV1, canonical::canonical_json, evaluate_payment_authorize,
    fixed_merchant_metadata_commitment, merchant_statement_descriptor_commitment,
};
use serde_json::json;

const NOW: u64 = 1_800_000_000;
const WORKFLOW: &str = "authorize-fixture-workflow";
...
fn main() {
    let output = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("fixtures/merchant-authorize/v1");
    fs::create_dir_all(&output).expect("fixture directory");
    let account = StripeAccountId::parse("acct_authorizefixture01").expect("account");
    let customer = CustomerId::parse("cus_authorizefixture000001").expect("customer");
    let method = PaymentMethodId::parse("pm_authorizefixture0000001").expect("method");
    ...
    let policy =
        StripeBoundedMerchantPaymentPolicyV1::new(StripeBoundedMerchantPaymentPolicyInput {
            policy_id: "authorize-fixture-policy".into(),
            valid_from: NOW - 60,
            ...
            maximum_authorization_age_seconds: 7 * 24 * 60 * 60,
    ...
    let action = StripeExactPaymentAuthorizeV1::new(StripeExactPaymentAuthorizeInput {
        ...
        executor_audience: "https://stripe-authorize.auths.dev".into(),
        expires_at: NOW + 120,
        nonce: auths_stripe::canonical::sha256(b"fixture-authorize-nonce"),
    })
    .expect("action");
    ...
    let decision = evaluate_payment_authorize(&PaymentAuthorizeEvaluationContext {
        ...
        request_audience: "https://stripe-authorize.auths.dev",
        now: NOW,
    });
    let decision_receipt = MerchantAuthorizationDecisionReceipt {
    ...
    let provider = MerchantProviderProjection {
        payment_intent_id: PaymentIntentId::parse("pi_authorizefixture00000001").expect("intent"),
        charge_id: Some(
            auths_stripe::ChargeId::parse("ch_authorizefixture00000001").expect("charge"),
        ),
        ...
        stripe_request_id: Some("req_authorizefixture01".into()),
    ...
    let held = store
        ...
        .expect("authorization hold");
    let transition = MerchantAuthorizationTransitionReceipt {
        ...
        semantic_event: "authorization-held".into(),
        resulting_state: held.state(),
    ...
    let observation = MerchantAuthorizationObservationReceipt {
    ...
    let fixtures = vec![
        ...
        (
            "authorized.json",
            serde_json::to_value(&held).expect("authorization hold"),
        ),
        (
    ...
    let denials = json!({
        "schema": "auths.stripe.payment-authorize-denial-codes/1",
        "codes": [
    ...
    let replay = json!({
        "schema": "auths.stripe.payment-authorize-replay/1",
