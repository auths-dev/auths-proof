# Auths Proof Exchange Protocol V1
...
libraries. `auths-model`, `auths-codec`, `auths-ports`, `auths-author`, and
`auths-verifier` must not depend on it.

## 3. Exact negotiation and semantic port

Before issuing a challenge, peers may exchange canonical
`ExchangeCapabilities` values containing supported exchange versions, exact
Auths protocol/profile bindings, and message limits. `negotiate_exact`
requires a caller-selected `ProfileBinding` to occur in both advertisements.
It never falls back to another profile version, and it selects the smaller
...
Client                                      Service
  ...
  |                                            | atomically claim replay/budget
  |                                            | execute or refuse
  |<-------------- ActionResponse --------------|
```
...
An offline file exchange may add an immutable acknowledgment after the
response. The acknowledgment binds the exchange sequence and SHA-256 digests
of the exact canonical submission and response. It supports deterministic
retry correlation and detects message substitution; it does not provide
confidentiality or exactly-once execution. Sensitive file exchanges require an
encrypted storage medium or a separately approved encrypted envelope.
...
and profile version. A mismatch is an exchange error before proof work. The
body remains outside the `ProofBundle`; the Auths action commits to its digest.
Applications canonicalize input and execute only the command decoded from the
sealed verified action.
...
Every transport uses the same closed deterministic CBOR messages. HTTPS maps
those bytes to the exact
`application/vnd.auths.exchange.v1+cbor` media type without preserving stream
framing.

V1 admits exactly one bounded action and one bounded response per exchange.
Batching, compression, generic multiplexing, and detached streaming bodies are
not silently negotiated. A future reviewed exchange version must define their
ordering, digest, cancellation, resource-accounting, and replay semantics
before an adapter exposes them.
...
Acceptable inside an already protected network boundary. The V1 raw TCP
adapter provides neither confidentiality nor peer authentication and is not
the recommended public deployment. TLS termination is represented by HTTPS
or by a future explicitly authenticated stream adapter, never inferred from a
TCP endpoint string.
...
### Transport property matrix

| Adapter | Confidentiality | Peer observation | Session/retry rule |
|---|---|---|---|
| memory | process-local only | configured test fact | one challenge and submission |
| Iroh | authenticated QUIC encryption | remote endpoint ID | full handshake, no authorization-bearing 0-RTT |
| HTTPS | TLS server authentication; optional mTLS observation | server or mutual-TLS certificate digest | one challenge followed by one POST |
| raw TCP | none | unauthenticated endpoint string | one bounded stream; external protection required |
| Unix socket | OS-local boundary | peer credentials on the service side | one bounded stream |
| file | none by itself | content digest and sequence, not an authenticated peer | immutable envelopes plus bound acknowledgment |

Every interactive adapter stores the exact challenge it issued or received
and rejects a submission, or a service callback, whose complete protocol,
profile, limits, audience, expiry, or nonce binding differs.
