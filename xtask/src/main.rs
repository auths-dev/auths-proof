...
const USAGE: &str = "usage: cargo xtask <profile <new|generate|check|qualification> ...|fmt|arch [--update]|semantic-freeze [--update]|evolution-policy [--update]|sdk-experience [--update]|sdk-vocabulary|error-registry [--update]|mcp-session-contract [--update]|mechanism-conformance [--update]|product-waist-conformance [--update]|public-naming|release-contract|release-control <finalize|compare|verify-promotion|canonicalize-qualification-build|verify-qualification-release-build> ...|binding-semantics|core-boundary|workspace-msrv|abi|core|exchange|product|bindings|demos|package|wire [--update]|spec-sync|conformance|exchange-conformance|product-conformance|stripe-profiles|bounded-domains|compliance|matrix|cross-language|product-fixtures [--update]|semantic-digest|wasm|live-demo|fuzz-inventory|fuzz-smoke|platform-artifact [output]|formal [--skip-kani] [--update]|formal qualify aeneas [--update]|adversarial-conformance [--surface <name>|--adapter <name>|--case <id>]|bench <prepare|run|report|compare|verify-artifact|bounded>|ci [preflight|authoritative|formal-translation|formal-post-qualification|compliance]|release-check>";
    ...
    match command.as_str() {
        ...
        "ci" => {
            ...
            match arguments.as_slice() {
                [] => ci(),
                [phase] if phase == "preflight" => ci_preflight(),
                [phase] if phase == "authoritative" => ci_authoritative(),
                [phase] if phase == "formal-translation" => ci_formal_translation(),
                [phase] if phase == "formal-post-qualification" => ci_formal_post_qualification(),
                [phase] if phase == "compliance" => ci_compliance(),
                _ => Err(format!(
                    "unknown CI phase {}; expected preflight, authoritative, formal-translation, formal-post-qualification, or compliance",
                    arguments.join(" ")
                )),
            }
        }
...
mod tests {
    ...
    fn help_output_is_stable() {
        assert_eq!(
            USAGE,
            "usage: cargo xtask <profile <new|generate|check|qualification> ...|fmt|arch [--update]|semantic-freeze [--update]|evolution-policy [--update]|sdk-experience [--update]|sdk-vocabulary|error-registry [--update]|mcp-session-contract [--update]|mechanism-conformance [--update]|product-waist-conformance [--update]|public-naming|release-contract|release-control <finalize|compare|verify-promotion|canonicalize-qualification-build|verify-qualification-release-build> ...|binding-semantics|core-boundary|workspace-msrv|abi|core|exchange|product|bindings|demos|package|wire [--update]|spec-sync|conformance|exchange-conformance|product-conformance|stripe-profiles|bounded-domains|compliance|matrix|cross-language|product-fixtures [--update]|semantic-digest|wasm|live-demo|fuzz-inventory|fuzz-smoke|platform-artifact [output]|formal [--skip-kani] [--update]|formal qualify aeneas [--update]|adversarial-conformance [--surface <name>|--adapter <name>|--case <id>]|bench <prepare|run|report|compare|verify-artifact|bounded>|ci [preflight|authoritative|formal-translation|formal-post-qualification|compliance]|release-check>"
        );
    }
    ...
    fn invalid_ci_phase_preserves_error_contract() {
        ...
        assert_eq!(
            error,
            "unknown CI phase unknown; expected preflight, authoritative, formal-translation, formal-post-qualification, or compliance"
        );
    }
}
