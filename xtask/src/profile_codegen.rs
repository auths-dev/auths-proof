...
fn write_new_profile_scaffold(
    ...
    } = &arguments.mode
    {
        ...
        fs::write(
            ...
            "//! TODO(auths): run the complete provider connection conformance suite.\n",
        )
        .map_err(|error| error.to_string())?;
    }
    write_profile_effect_scaffold(arguments, package_root)
...
impl QualificationProtectedSetup for {adapter} {{\n\
    fn metadata(&self) -> QualificationAdapterMetadata {{ metadata() }}\n\
    fn setup(&self, _input: QualificationProtectedSetupInput<'_>, _setup_credential: &[u8]) -> Result<QualificationSetupHandoffV1, QualificationHarnessError> {{ Err(unavailable()) }}\n\
}}\n\n\
...
fn write_profile_effect_scaffold(
    ...
    for name in modules {
        fs::write(
            ...
            format!("//! TODO(auths): implement the exact {effect}/{version} {name} contract.\n"),
        )
        .map_err(|error| error.to_string())?;
    }
    ...
    for name in [
        ...
        fs::write(
            path,
            format!("//! TODO(auths): implement {effect}/{version} {name} evidence.\n"),
        )
        .map_err(|error| error.to_string())?;
    }
...
fn write_profile_reference_scaffold(
    ...
    if !specification.exists() {
        ...
        fs::write(
            &specification,
            format!(
                "# {domain} {effect} profile\n\nStatus: specified, not qualified.\n\n### 1.1 Prerequisite implementation closure\n\nTODO(auths): replace this file with the complete bounded semantic contract required by AP-SPEC-040.\n"
            ),
        )
        .map_err(|error| error.to_string())?;
    }
    if let NewProfileMode::Connected { provider, .. } = &arguments.mode {
        ...
        if !connection.exists() {
            fs::write(
                connection,
                format!(
                    "# {provider} connection\n\nStatus: specified, not qualified.\n\nTODO(auths): define immutable account identity, onboarding, least-privilege scopes, rotation, revocation, and reconciliation credential behavior.\n"
                ),
            )
            .map_err(|error| error.to_string())?;
        }
    ...
    if !live_contract.exists() {
        fs::write(
            live_contract,
            "//! TODO(auths): prove one real provider effect, replay, denial, recovery, and receipt verification.\n",
        )
        .map_err(|error| error.to_string())?;
    }
...
fn render_root_profile_roster(repository: &Path, roster: &ProfileRoster) -> Result<String, String> {
    ...
    output.push_str("        }\n    }\n\n");
    output.push_str("    pub(crate) fn hold_provider_entry(self, context: &LocalOperationContext, record: &JournalRecordV1, now_unix_seconds: u64) -> Result<(), ProfileRuntimeError> {\n        let profile = self.profile().map_err(|_| ProfileRuntimeError::Invalid)?;\n        let context = self.operation_context(context, &profile);\n        match self {\n");
    for registration in &registrations {
        writeln!(output, "            Self::{} => {}::local_agent::{}_hold_provider_entry(ProviderEntryHoldInput {{ context, record, now_unix_seconds }}),", registration.16, registration.2, registration.17)
            .map_err(|error| error.to_string())?;
    }
    output.push_str("        }\n    }\n\n");
...
use auths_profile_kit::{QualificationEffect, QualificationHarnessError, QualificationOperationRole, QualificationProtectedObserver, QualificationProtectedSetup, QualificationProtectedSetupInput, QualificationProviderTruth, QualificationRedactedOperation, QualificationScenarioProgramV1, QualificationSetupHandoffV1};\n\n",
    );
...
pub(crate) fn validate_domain_scenario(domain: &str, program: &QualificationScenarioProgramV1, operations: &[QualificationRedactedOperation], truths: &[QualificationProviderTruth]) -> Result<(), String> {\n\
    ...
    for entry in roster.packages() {
        ...
        writeln!(
            output,
            "        {:?} => {crate_name}::qualification::{adapter}.validate_domain_scenario(program, operations, truths),",
            entry.domain()
        )
        .map_err(|error| error.to_string())?;
    }
...
use auths_connections::{ProviderCredentialLease, QualificationProviderCallKind};\n\
use auths_profile_kit::{QualificationEffect, QualificationHarnessError, QualificationProfileStateFactV1, QualificationScenarioProgramV1};\n\
use auths_profile_runtime::{ProfileReceiptInspection, ProfileRuntimeError};\n\
use auths_stores::JournalRecordV1;\n\
use std::path::Path;\n\n\
#[derive(Clone, Copy, Debug, Eq, PartialEq)]\n\
pub(crate) enum QualificationRoute {\n",
    ...
    output.push_str("            _ => Err(\"profile is absent from the generated qualification route roster\".into()),\n        }\n    }\n\n    pub(crate) fn scenario_program(self, scenario_id: &str) -> Result<QualificationScenarioProgramV1, String> {\n        match self {\n");
    for entry in roster.packages() {
        let crate_name = entry.rust_package().replace('-', "_");
        writeln!(
            output,
            "            Self::{} => {crate_name}::qualification::qualification_scenario_program(scenario_id).map_err(|error| error.to_string()),",
            pascal(entry.domain())
        )
        .map_err(|error| error.to_string())?;
    }
