...
fn generate_inventory() -> Result<SemanticFreezeInventory, String> {
    ...
    let mut entries = vec![
        ...
        freeze_entry(
            "auths.product.documentation-surface",
            1,
            FreezeClassification::FrozenMeaning,
            &[
                ...
                "documentation-contract-schema",
            ],
            vec![
                "release/docs".to_owned(),
                "product/spec/v1/auths-docs-contract.schema.json".to_owned(),
                "xtask/src/docs_contract.rs".to_owned(),
            ],
        )?,
        freeze_entry(
    ...
    for (id, path) in frozen_byte_inventories()? {
        let version = match path.as_str() {
            "architecture/dependency-graph.json" => 19,
