...
export function renderGenerated(contract) {
  ...
  return `/** Generated by auths; edit profile.toml, then regenerate. */\n` +
...
async function approveMain(args) {
  const usage = "usage: auths approve <request-file-or-text> --signer <custody-config> [--decline] [--yes] [--out <file>]";
  let request;
  let signerPath;
  let out;
  let declining = false;
  ...
  try {
    ...
    try {
      ...
      process.stderr.write(`auths: request refused: ${error.code}; nothing was signed\n`);
      process.exitCode = 1;
      return;
    }
    ...
    if (yes) confirmed = true;
      ...
      process.stderr.write("auths: no terminal to confirm on; pass --yes to answer without a prompt. Nothing was signed.\n");
      process.exitCode = 2;
      return;
    } else {
    ...
    try {
      ...
      if (error instanceof sdk.ApprovalRefused) {
        ...
        process.stderr.write(`auths: custody ${error.kind}: ${error.code}\n`);
      } else throw error;
      process.exitCode = 1;
      return;
    }
...
async function mainText(args) {
  ...
  } else throw new Error("usage: auths init|derive|generate|check|diff|doctor|test");
}
...
if (process.argv[1] && realpathSync(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main(process.argv.slice(2)).catch(error => {
    process.stderr.write(`auths: ${error instanceof Error ? error.message : "unknown failure"}\n`);
    process.exitCode = 1;
  });
}
