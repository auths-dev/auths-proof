...
const { createVerifier } = await import(new URL("dist/verify.js", root));
const verifier = await createVerifier();
const coldStartMs = performance.now() - started;
const input = {
  proof: await readFile(new URL("../../core/fixtures/v1/valid/raw-key-chain.proof.cbor", root)),
  action: await readFile(new URL("../../core/fixtures/v1/valid/raw-key-chain.action.cbor", root)),
  trustedContext: await readFile(new URL("authorized.context.cbor", vectorRoot)),
};
const timings = [];
...
if (process.argv.includes("--print")) {
  ...
  assert.equal(baseline.schema, "auths.performance/2");
