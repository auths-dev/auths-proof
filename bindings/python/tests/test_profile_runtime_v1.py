...
class FakeLocalClient:
    def __init__(
        ...
        *,
        ...
        fail_evidence_response: bool = False,
        malformed_evidence_response: bool = False,
        malformed_prepare_response: bool = False,
        timeout_prepare_response: bool = False,
        cancel_evidence_response: bool = False,
        cancel_prepare_response: bool = False,
        recover_not_applied: bool = False,
        integrity_outcome: dict[int, Any] | None = None,
        evidence_outcome: dict[int, Any] | None = None,
    ) -> None:
        self.digest = digest
...
def test_profile_file_rejects_oversize_symlink_and_non_file(
    ...
    with pytest.raises(ValueError, match="generated bound"):
        _encode_profile_input(
            FILE_PROFILE_API,
            "FileInput",
            FileInput(ProfileFile(oversize)),
            FileInput,
        )
    ...
    with pytest.raises(TypeError, match="regular non-symlink"):
        _encode_profile_input(
            FILE_PROFILE_API,
            "FileInput",
            FileInput(ProfileFile(tmp_path)),
            FileInput,
        )
...
def test_declared_preparation_evidence_is_retried_exactly_and_hidden_from_api() -> None:
    async def scenario() -> None:
        ...
        result = await profile.invoke(
            Input(7), OperationOptions(idempotency_key="same")
        )
        assert result.doubled == 14
        evidence = [
            item
            for item in client.requests
            if item[1].endswith("/preparation-evidence")
        ]
        assert len(evidence) == 2
        assert evidence[0][2] == evidence[1][2]
        prepare = next(
            item for item in client.requests if item[1].endswith("/operations")
        )
...
) -> BoundProfile[Result, NoReturn, NoReturn]:
...
def test_companion_outcomes_resume_the_normal_state_machine() -> None:
    async def scenario() -> None:
        ...
        cases = (
            ...
            (
                _integrity_wire("completed", "applied", True),
                ("preparation-evidence",),
                ReceiptIntegrityFailed,
            ),
        )
        for outcome_wire, expected_tail, expected_type in cases:
            ...
            assert (
                tuple(path.rsplit("/", 1)[-1] for _, path, _ in client.requests)
                == expected_tail
            )
            if isinstance(outcome, Unavailable):
                ...
                assert (
                    outcome.issue.entered_boundaries
                    == outcome.issue.entered_boundaries.__class__(
                        False,
                        False,
                        False,
                        False,
...
def test_recovery_only_blocks_new_effects_and_preserves_conservative_recovery() -> None:
    async def scenario() -> None:
        ...
        client._install_session(
            {
                1: 1,
                2: request_id,
                3: "ses_AQEBAQEBAQEBAQEBAQEBAQ",
                4: "raw:test-principal",
                5: b"x" * 32,
                6: [
                    {
                        1: "auths.example.double",
                        2: 1,
                        3: digest,
        ...
        operation = "op_" + "A" * 22
        recovery = RecoveryHandle.from_bytes(
            encode(
                {
                    1: 1,
                    2: operation,
        ...
        async def completed_recovery(
            method: str, path: str, body: bytes, timeout: object
        ) -> bytes:
            wire = decode(body)
            return encode(
                {
        ...
        async def foreign_handle_recovery(
            method: str, path: str, body: bytes, timeout: object
        ) -> bytes:
            wire = decode(body)
            return encode(
        ...
        assert (
            unavailable.value.info.entered_boundaries
            == unavailable.value.info.entered_boundaries.__class__(
                False,
                False,
                False,
                False,
            ...
            6: [
                {
                    ...
                    6: {
                        1: "example",
                        2: "auths.example.connection/1",
                        3: "auths.example.connection-descriptor/1",
                    },
                    7: None,
                }
            ],
...
def test_sdk_admission_is_bounded_fifo_cancellable_and_reserves_control() -> None:
    async def scenario() -> None:
        ...
        assert _is_reserved_sdk_request(
            "POST",
            "/v1/profiles/example/double/1/operations/op_" + "A" * 22 + "/recover",
        )
        assert not _is_reserved_sdk_request(
            "POST", "/v1/profiles/example/double/1/operations"
        )
        assert not _is_reserved_sdk_request(
            "POST",
            "/v1/profiles/example/double/1/operations/op_A/recover",
        )
...
def test_profile_invocation_coordination_is_bounded_fresh_and_promotable() -> None:
    async def scenario() -> None:
        ...
        statuses = [
            asyncio.create_task(
                client._profile_invocation_status(
                    ticket, "/status", timedelta(seconds=1)
                )
            )
        ...
        identities = await asyncio.gather(
            *(ticket.entry.identity for ticket in (*followers, observer))
        )
        assert all(
            identity == (request_a, operation_id, b"") for identity in identities
        )
        ...
        failed = client._begin_profile_invocation(
            scope + ":promotion", b"f" * 32, request_a
        )
        first = client._begin_profile_invocation(
            scope + ":promotion", b"f" * 32, request_b
        )
        second = client._begin_profile_invocation(
            scope + ":promotion", b"f" * 32, b"e" * 16
        )
        ...
        promoted = client._begin_profile_invocation(
            scope + ":promotion", b"f" * 32, request_b
        )
        attached = client._begin_profile_invocation(
            scope + ":promotion", b"f" * 32, request_a
        )
...
def test_generated_profile_coalesces_exact_keys_and_preserves_cancelled_follower_truth() -> (
    None
):
    async def scenario() -> None:
        digest = b"z" * 32
...
def test_pending_rows_are_exact_identity_bound_and_strictly_ordered() -> None:
    ...
    wrong_truth = row(first, 10)
    wrong_truth[5] = "possible"
    hostile.append(wrong_truth)
    wrong_handle = row(second, 10)
    wrong_handle[9] = recovery_bytes(first)
    hostile.append(wrong_handle)
    for item in hostile:
    ...
    unavailable = _status_from_outcome(
        {
            ...
            3: request_id,
            4: first,
            5: _not_applied_wire(request_id)[5],
            6: [],
            7: "primary",
        },
        (first, "auths.example.double", 1),
        request_id,
    )
...
def test_post_write_cancellation_preserves_coalesced_applied_or_possible_truth() -> (
    None
):
    async def scenario() -> None:
        completed = {
...
def test_malformed_responses_are_exactly_replayed_inside_the_retry_boundary() -> None:
    async def scenario() -> None:
        ...
        assert (
            sum(
                path.endswith("/preparation-evidence")
                for _, path, _ in companion.requests
            )
            == 2
        )
        ...
        assert (
            sum(path.endswith("/operations") for _, path, _ in prepared.requests) == 2
        )
...
def test_receipt_integrity_outcome_rejects_identity_and_truth_mismatch() -> None:
    async def scenario() -> None:
        malformed = (
            ...
            _integrity_wire(
                "completed", "applied", True, correlation="op_BBBBBBBBBBBBBBBBBBBBBB"
            ),
