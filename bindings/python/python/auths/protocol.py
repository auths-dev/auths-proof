...
_MEDIA: _Literal["application/vnd.auths.remote-verification.v1+cbor"] = (
...
def _head(major: int, value: int) -> bytes:
    ...
    if value <= 0xFFFF:
        return bytes([(major << 5) | 25]) + _struct.pack(">H", value)
    if value <= 0xFFFFFFFF:
        return bytes([(major << 5) | 26]) + _struct.pack(">I", value)
...
def _decode(data: bytes) -> _Any:
    ...
    def take(offset: int) -> tuple[_Any, int]:
        ...
        if additional < 24:
            ...
            length = _struct.unpack_from(">H", view, offset)[0]
            offset += 2
        elif additional == 26:
            length = _struct.unpack_from(">I", view, offset)[0]
            offset += 4
        elif additional == 27:
            length = _struct.unpack_from(">Q", view, offset)[0]
            offset += 8
        ...
        if major == 4:
            ...
            for _ in range(length):
                value, offset = take(offset)
                values.append(value)
            return values, offset
        if major == 5:
            values = {}
            for _ in range(length):
                key, offset = take(offset)
                value, offset = take(offset)
                if key in values:
                    raise ValueError
                values[key] = value
...
class _HttpTransport:
    ...
    async def send(self, request: TransportRequest) -> TransportResponse:
        remaining = max(
            0.001, (request.deadline_unix_ms - int(_time.time() * 1000)) / 1000
        )
        value = _urlrequest.Request(
            request.url,
            data=request.body,
            method="POST",
            headers={
                ...
                "Auths-Error-Registry-SHA256": _runtime_info().error_registry_digest,
            },
        )

        def send() -> TransportResponse:
            opener = _urlrequest.build_opener(
                _urlrequest.HTTPSHandler(context=_ssl.create_default_context()),
                _NoRedirect(),
            )
            with opener.open(value, timeout=remaining) as response:
                ...
                return TransportResponse(
                    response.status, response.headers.get_content_type(), body
                )
...
class _NoRedirect(_urlrequest.HTTPRedirectHandler):
    def redirect_request(
        self, req: _Any, fp: _Any, code: int, msg: str, headers: _Any, newurl: str
    ) -> None:
...
class RemoteVerifier:
    def __init__(
        ...
        if token is not _TOKEN:
            raise TypeError("RemoteVerifier is sealed")
        parsed = _urlparse.urlsplit(endpoint)
        loopback = parsed.hostname in ("127.0.0.1", "::1")
        if (
            (
                parsed.scheme != "https"
                and not (insecure and parsed.scheme == "http" and loopback)
            )
        ...
        if not 0.001 <= seconds <= 300 or seconds * 1000 != int(seconds * 1000):
            raise ValueError("timeout outside bounds")
        if not 1024 <= maximum <= 16 * 1024 * 1024:
            raise ValueError("maximum response outside bounds")
        self._url, self._transport, self._owns, self._timeout, self._maximum = (
            parsed.scheme + "://" + parsed.netloc + "/v2/verification/authorize",
            transport,
            owns,
            seconds,
            maximum,
    ...
    async def verify(self, input: VerificationInput, /) -> VerificationResult:
        ...
        request = TransportRequest(
            ...
            _MEDIA,
            body,
            int((_time.time() + self._timeout) * 1000),
            self._maximum,
        )
        try:
            response = await _asyncio.wait_for(
                self._transport.send(request), self._timeout
            )
        except _asyncio.TimeoutError:
            raise _auths_error("remote.timeout")
        except Exception as exc:
            raise _auths_error("remote.transport-unavailable", summary=str(exc)[:256])
        if (
            response.status != 200
            or response.media_type.split(";", 1)[0].strip() != _MEDIA
        ):
            raise _auths_error("remote.response-malformed")
        try:
            raw = _decode(response.body)
            if raw[0] != 1 or raw[4] != correlation or raw[10] != digest:
                raise ValueError
            kinds, stages = (
            ...
            metrics = VerificationMetrics(
                *(int(metrics_raw[index]) for index in range(7))
            )
            code = str(raw[2])
            ...
            negative_kind: _Literal["denied", "indeterminate"] = (
                "denied" if kind == "denied" else "indeterminate"
            )
            return UnsuccessfulVerification(
                ...
                executed,
                decision,
                _error_info(raw[9].get("code", "core.authorization-indeterminate")),
            )
        except Exception:
            raise _auths_error("remote.response-malformed")
...
def connect_remote_verifier(
    *,
    endpoint: str,
    access_token: str,
    timeout: _timedelta = _timedelta(seconds=30),
    maximum_response_bytes: int = 8 * 1024 * 1024,
    allow_insecure_loopback: bool = False,
) -> RemoteVerifier:
    if not access_token or len(access_token.encode()) > 8192:
        raise ValueError("invalid access token")
    return RemoteVerifier(
        _TOKEN,
        endpoint,
        _HttpTransport(access_token),
        True,
        timeout,
...
def remote_verifier_from_transport(
    ...
    *,
    ...
    if transport.contract != "bounded-byte-transport/2":
        raise ValueError("unsupported bounded transport contract")
    return RemoteVerifier(
        _TOKEN,
        endpoint,
        transport,
...
__all__ = [
    ...
    "BoundedTransport",
    "RemoteVerifier",
    "connect_remote_verifier",
    "remote_verifier_from_transport",
]
