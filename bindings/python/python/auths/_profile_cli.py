...
def _python_type(field: FieldSpec) -> str:
    ...
    # The generated consumer still supports Python 3.9, even if its own Ruff
    # target is 3.10 or newer and suggests the newer union spelling.
    return f"Optional[{base}]  # noqa: UP045" if field.kind.startswith("optional-") else base
...
def render_generated(contract: ProfileContract) -> str:
    ...
    for field in contract.fields:
        if field.kind.endswith("string"):
            names.add("StringField")
        elif field.kind.endswith("integer"):
            names.add("IntegerField")
        else:
            names.add("BooleanField")
        if field.kind.startswith("optional-"):
            names.add("OptionalField")
    typing_names = "Optional, TypeVar" if any(
        field.kind.startswith("optional-") for field in contract.fields
    ) else "TypeVar"
    lines = [
        ...
        *(f"    {field.name}: {_python_type(field)}" for field in contract.fields),
        "",
        "",
        "FIELDS = {",
    ...
    try:
        if args.action == "init":
            ...
            if any(
                ...
                for name in ("profile.toml", "generated.py", "vectors.json")
            ):
