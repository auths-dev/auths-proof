...
from ._native import (
    ...
    validate_receipt_anchor_v1,
    verify_many_v1,
    verify_pinned_receipt_v1,
...
class ReceiptTrustPolicy:
    def __init__(
        self,
        token: object,
        anchors: Tuple[ReceiptTrustAnchor, ...],
        profiles: Tuple[ReceiptProfile, ...],
        verification_time: Optional[int],
        maximum_age: int,
    ) -> None:
...
def pinned_receipt_trust(
    *,
    ...
    profiles = tuple(
        ReceiptProfile(value.id, value.version) for value in allowed_profiles
    )
    if not 1 <= len(anchor_values) <= 32 or not any(
        value.role == "decision" for value in anchor_values
    ):
        raise ValueError(
            "receipt trust requires 1..32 anchors including a decision anchor"
        )
    ...
    if (
        verification_time_unix_seconds is not None
    ...
    maximum_age = (
        86_400 if maximum_receipt_age_seconds is None else maximum_receipt_age_seconds
    )
...
class VerifiedReceipt:
    ...
    def __init__(
        self, token: object, receipt: Receipt, details: ReceiptEnvelopeDetails
    ) -> None:
...
class _Verifier:
    ...
    def verify(
        self, input: VerificationInput, *, correlation_id: Optional[str] = None
    ) -> VerificationResult:
    ...
    def verify_many(
        self, inputs: Iterable[VerificationInput], *, chunk_size: int = 32
    ) -> Tuple[VerificationResult, ...]:
        values = tuple(inputs)
        if not 1 <= len(values) <= 256 or not 1 <= chunk_size <= 256:
            raise ValueError("verification batch is outside bounds")
        native = verify_many_v1(
            [(value.proof, value.action, value.trusted_context) for value in values]
        )
        return tuple(
            _project(value, f"auths-{time.time_ns():x}-{index}")
    ...
    def inspect(self, result: VerificationResult) -> VerificationInspection:
        ...
        return VerificationInspection(
            ...
            result.code,
            result.stage,
            digest(b"result", result.decision_bytes),
            digest(b"action", result.decision_bytes)
            if result.kind == "authorized"
            else None,
            None
            if result.required_configuration is None
            else digest(b"required", result.required_configuration),
            digest(b"executed", result.executed_configuration),
            result.metrics,
            None,
        )

    def verify_receipt(
        self, *, receipt: Union[Receipt, bytes], trust: ReceiptTrustPolicy
    ) -> ReceiptVerification:
        ...
        try:
            ...
            source_bytes = (
                receipt.to_bytes() if isinstance(receipt, Receipt) else receipt
            )
            (
                kind,
                portable_id,
                ...
                attested_decision,
                attested_execution,
            ) = parse_portable_receipt(source_bytes)
            source = (
                receipt
                if isinstance(receipt, Receipt)
                else mint_receipt(portable_id, source_bytes)
            )
            ...
            if not any(
                value.id == profile["id"] and value.version == profile["version"]
                for value in trust.allowed_profiles
            ):
                return RejectedReceipt(
                    "rejected", error_info("core.receipt-profile-denied")
                )
            ...
            if now is None:
                try:
                    ...
                    return IndeterminateReceipt(
                        "indeterminate", error_info("core.receipt-trust-indeterminate")
                    )
            decided_at = int(decision_metadata["decidedAtUnixSeconds"])
            completed_at = (
                decided_at
                if kind == "decision"
                else int(metadata["completedAtUnixSeconds"])
            )
            if (
                decided_at > now + 300
                or completed_at > now + 300
                or now - decided_at > trust._maximum_age
            ):
            ...
            if kind == "decision":
                details: ReceiptEnvelopeDetails = DecisionReceiptDetails(
                    ...
                    tuple(decision_metadata["reasons"]),
                    decided_at,
                    decision_signer,
                    common["proof"],
                    common["action"],
                    common["context"],
                    common["principalStatus"],
                    common["grantStatus"],
                )
            else:
                ...
                details = ExecutionReceiptDetails(
                    ...
                    completed_at,
                    decision_signer,
                    _signer(metadata["executionSigner"]),
                    common["proof"],
                    common["action"],
                    common["context"],
                    common["principalStatus"],
                    common["grantStatus"],
                    execution_commitments["executionLease"],
                    execution_commitments["command"],
                    execution_commitments.get("result"),
                )
...
def _verify_with_anchors(
    ...
    for anchor in trust._anchors:
        ...
        attempted = True
        try:
            encoded = verify_pinned_receipt_v1(
                kind,
                attested,
                receipt_id,
                anchor.principal,
...
def _project(
    native: NativeVerificationResult, correlation_id: str
) -> VerificationResult:
    ...
    common = (
        ...
        correlation_id,
        metrics,
        native.required_configuration,
        bytes(native.local_configuration),
        bytes(native.result_cbor),
    )
    ...
    code = (
        ...
        else "core.authorization-indeterminate"
    )
    return UnsuccessfulVerification(
        native.kind, *common, error_info(code, correlation_id=correlation_id)
    )


def _signer(value: Dict[str, Any]) -> ReceiptSignerInfo:
    return ReceiptSignerInfo(
        value["principal"], value["verificationMethod"], value["suite"]
    )
