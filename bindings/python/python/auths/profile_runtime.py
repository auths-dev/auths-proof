...
from ._session import (
    ...
    _not_applied_error,
    _partial_error,
    _recovery_required_error,
    _report_qualification_cancellation,
    _report_qualification_result,
...
class BoundProfile(Generic[_T, _P, _G]):
    def __init__(
        ...
        _bounded_integer(
            descriptor.response_bytes, 1, 64 * 1024 * 1024, "response_bytes"
        )
        _bounded_integer(
            descriptor.execution_milliseconds, 1, 300_000, "execution_milliseconds"
        )
        _bounded_integer(descriptor.receipt_count, 1, 64, "receipt_count")
        _bounded_integer(
            descriptor.receipt_bytes, 1, descriptor.response_bytes, "receipt_bytes"
        )
    ...
    async def invoke_outcome(
        ...
        try:
            ...
            if retained is not None and retained[0] is outcome:
                await _complete_qualification_handoff(
                    _report_qualification_result(
                        self._client,
                        self._descriptor.profile_id,
    ...
    async def _qualification_invoke_encoded_outcome(
        ...
        """
        if (
            self._client._qualification_socket_for(
                self._descriptor.profile_id,
                self._descriptor.version,
            )
            is None
        ...
        if (
            not isinstance(encoded_input, bytes)
            or not 1 <= len(encoded_input) <= 16_777_216
        ):
        ...
        try:
            selected = _profile_operation_options(
                options,
                self._descriptor.execution_milliseconds,
            )
            deadline = (
                asyncio.get_running_loop().time() + selected.timeout.total_seconds()
            )
            outcome = await self._invoke_leader(
                os.urandom(16),
                selected.idempotency_key,
                encoded_input,
                deadline - selected.recovery_wait.total_seconds(),
                deadline,
                selected.recovery_wait,
                None,
            )
            retained = _QUALIFICATION_RESULT.get()
            if retained is not None and retained[0] is outcome:
    ...
    async def _invoke_outcome_inner(
        ...
        selected = _profile_operation_options(
            options, descriptor.execution_milliseconds
        )
        ...
        capability = self._client._profile_capability(
        ...
        if (
            ...
            raise _unavailable_error(
                _local_issue(
                    ...
                    "profile contract digest mismatch",
                ),
                None,
                (),
            )
        ...
        if selected.idempotency_key is None or not isinstance(self._client, Client):
            return await self._invoke_leader(
                ...
                selected.idempotency_key,
                encoded_input,
                advancement_deadline,
                deadline,
                selected.recovery_wait,
                None,
            )
        fingerprint = hashlib.sha256(
            _encode({1: encoded_input, 2: self._connection})
        ).digest()
        scope = (
            f"{descriptor.profile_id}/{descriptor.version}:{selected.idempotency_key}"
        )
        while True:
            ...
            if ticket.role in ("follower", "observer"):
                try:
                    identity, cancellation_observed = await _wait_coordinated_identity(
                        ticket, deadline
                    )
                    if identity is None:
                        ...
                        continue
                    outcome = await self._observe_coordinated(
                        ticket, identity, deadline
                    )
    ...
    async def _invoke_leader(
        ...
        try:
            ...
            (
                ...
                replayed_outcome,
                cancellation,
            ) = await self._acquire_preparation_evidence(
                request_id,
                idempotency_key,
                encoded_input,
                advancement_deadline,
            ...
            try:
                raw = await _profile_request(
                    self._client,
                    "POST",
                    self._descriptor.collection_route,
                    prepare,
                    _remaining_timeout(advancement_deadline),
                    coordination,
                )
                wire = _profile_outcome(raw, request_id, self._descriptor)
            except BaseException as error:
                if not _is_post_write_request_error(error) and not isinstance(
                    error, ValueError
                ):
                    raise
    ...
    def _project_conflict_probe(
        ...
        if (
            wire.get(2) not in ("conflict", "receipt-integrity-failed")
        ...
        _bind_recovery_response(
            wire,
            (expected_operation, self._descriptor.profile_id, self._descriptor.version),
            self._descriptor,
        )
        ...
        if isinstance(projected, Conflict) and (
            ...
            raise ValueError(
                "changed idempotency intent returned an unrelated conflict"
            )
        return projected
        ...
        if initial_bytes:
            try:
                ...
                if initial.get(2) == "ready":
                    ...
                    fallback = (
                        recovery,
                        _receipt_ids([initial.get(6)], self._descriptor),
                    )
                elif initial.get(2) == "in-progress":
                    recovery = _bytes(initial.get(8))
                    _assert_recovery_identity(recovery, expected)
                    fallback = (recovery, ())
                    _validate_in_progress(initial, self._descriptor)
                    receipts = _receipt_id_list(
                        initial.get(7), self._descriptor.receipt_count
                    )
        ...
        while True:
            ...
            try:
                raw = await asyncio.wait_for(
                    self._client._profile_invocation_status(
                        ticket,
                        f"{self._descriptor.collection_route}/{operation}",
            ...
            try:
                ...
                if wire.get(2) == "ready":
                    ...
                    receipts = _receipt_id_list(
                        wire.get(7), self._descriptor.receipt_count
                    )
    ...
    async def _drive_outcome(
        ...
        if wire.get(2) == "ready":
            ...
            _assert_recovery_identity(
                prepared_recovery,
                (operation, descriptor.profile_id, descriptor.version),
            )
            try:
                commitment = wire.get(5)
                if not isinstance(commitment, bytes) or len(commitment) != 32:
                ...
                return _recovery_required(
                    operation,
                    RecoveryHandle.from_bytes(prepared_recovery),
                    (),
                )
            ...
            try:
                raw = await self._client._request(
                    "POST",
                    f"{descriptor.collection_route}/{operation}/execute",
                    execute,
                    _remaining_timeout(advancement_deadline),
                )
        ...
        if wire.get(2) == "in-progress":
            ...
            try:
                ...
                return _recovery_required(
                    operation, RecoveryHandle.from_bytes(recovery), ()
                )
            return await self._wait_for_accepted_execute(
                request_id,
                operation,
    ...
    async def _acquire_preparation_evidence(
        ...
        route = (
            self._descriptor.collection_route.removesuffix("/operations")
        ...
        try:
            ...
            handle, outcome = self._decode_preparation_evidence_response(
                raw, request_id
            )
        except BaseException as error:
            if not _is_post_write_request_error(error) and not isinstance(
    ...
    async def _wait_for_accepted_execute(
        ...
        while wire.get(2) == "in-progress":
            ...
            if remaining <= 0:
                return _recovery_required(
                    operation,
                    RecoveryHandle.from_bytes(recovery),
                    receipts,
                )
            try:
                await asyncio.sleep(min(0.025, remaining))
    ...
    async def _recover_outcome_inner(
        ...
        if identity[1] != descriptor.profile_id or identity[2] != descriptor.version:
    ...
    def _project(self, wire: Dict[int, Any]) -> ProfileOutcome[_T, _P, _G]:
        ...
        if kind == "completed":
            ...
            metadata = OperationMetadata(
                operation,
                f"{self._descriptor.profile_id}/{self._descriptor.version}",
                _optional_text(wire.get(8)),
                completion,
                receipts,
            )
            value = _decode_profile_result(
                self._descriptor.profile_api,
                self._descriptor.success_type,
                wire.get(5),
                self._success_class,
                metadata,
            )
        ...
        if kind == "denied":
            ...
            return Denied(
                _OUTCOME_TOKEN,
                "denied",
                _operation_id(wire.get(4)),
                _issue_for_effect(wire.get(5), EffectState.NOT_APPLIED),
                _receipt_ids([wire.get(6)], self._descriptor),
            )
        if kind == "unavailable":
            _optional_text(wire.get(7))
        ...
        if kind == "conflict":
            ...
            return Conflict(
                ...
                "conflict",
                _operation_id(wire.get(4)),
                _issue_for_effect(wire.get(5), EffectState.POSSIBLE),
                _recovery(wire.get(6)),
                _receipt_ids(wire.get(7), self._descriptor),
            )
        if kind == "not-applied":
            _optional_text(wire.get(8))
            return NotApplied(
        ...
        if kind == "partial":
            ...
            details = _decode_profile_result(
                self._descriptor.profile_api,
                self._descriptor.partial_type,
                wire.get(5),
                self._partial_class,
                None,
            )
            return Partial(
                ...
                "partial",
                _operation_id(wire.get(4)),
                _issue_for_effect(wire.get(6), EffectState.APPLIED),
                details,
                _receipt_ids(wire.get(7), self._descriptor),
        ...
        if kind == "recovery-required":
            ...
            if wire.get(8) is not None:
                if (
                    self._progress_class is None
                    or self._descriptor.progress_type is None
                ):
                    raise ValueError("profile returned undeclared recovery progress")
                progress = _decode_profile_result(
                    self._descriptor.profile_api,
                    self._descriptor.progress_type,
                    wire.get(8),
                    self._progress_class,
                    None,
                )
        ...
        if kind == "receipt-integrity-failed":
            ...
            if (
                ...
                or not _integrity_provider_boundary(
                    state,
                    effect,
                    parsed.entered_boundaries.provider,
                )
            ):
...
def bind_profile(
    ...
    *,
    ...
    descriptor = ProfileDescriptor(
        ...
        collection_route,
        profile_client_runtime,
        _digest(runtime_contract_digest),
        _digest(error_projection_digest),
...
def _raise_outcome(outcome: object) -> None:
    ...
    if isinstance(outcome, Conflict):
        raise _conflict_error(
            outcome.issue, outcome.operation_id, outcome.receipt_ids, outcome.recovery
        )
    if isinstance(outcome, NotApplied):
        raise _not_applied_error(
            outcome.issue, outcome.operation_id, outcome.receipt_ids
        )
...
def _encode_profile_input(
    api: Mapping[str, object], type_name: str, value: object, expected: Type[object]
) -> bytes:
...
def _valid_integrity_truth(
    ...
    if state == "executing":
        return (
            effect in (EffectState.NOT_APPLIED, EffectState.POSSIBLE) and not terminal
        )
    ...
    return (
        state in ("completed", "partial") and effect is EffectState.APPLIED and terminal
    )
...
def _outcome(value: bytes, expected_request: bytes) -> Dict[int, Any]:
    ...
    if (
        maximum is None
        or set(raw) != set(range(1, maximum + 1))
        or raw.get(3) != expected_request
    ):
        raise ValueError(
            "Auths profile outcome has unknown, missing, or mismatched fields"
        )
    return raw
...
def _recovery_required(
    ...
    return RecoveryRequired(
        ...
        "recovery-required",
        operation_id,
        _outcome_unknown_issue(operation_id),
        recovery,
        receipt_ids,
        None,
    )
...
def _recovery_unavailable(
    ...
    return RecoveryRequired(
        ...
        "recovery-required",
        operation_id,
        _recovery_unavailable_issue(operation_id),
        recovery,
        receipt_ids,
        None,
    )
...
def _outcome_unknown_issue(operation_id: str) -> ErrorInfo:
    return ErrorInfo(
        ...
        "execute",
        "provider",
        "the provider outcome remains unknown; recover this operation",
        operation_id,
...
def _bind_recovery_response(
    ...
    raw_handle = (
        ...
        if kind in ("conflict", "recovery-required")
        else None
    )
...
async def _wait_coordinated_identity(
    ...
    resolution = asyncio.create_task(
        asyncio.wait_for(asyncio.shield(ticket.entry.identity), remaining)
    )
...
def _alphabet(value: str, alphabet: str) -> None:
    patterns = {
        "ascii-graphic": r"[!-~]*",
        "registered-token": r"[A-Za-z0-9][A-Za-z0-9._:-]*",
        "lower-token": r"[a-z][a-z0-9-]*",
        "lower-hex": r"[0-9a-f]+",
        "base64url": r"[A-Za-z0-9_-]+",
    }
    if alphabet == "utf8":
        if any(
            ord(char) == 0 or ord(char) < 0x20 or 0x7F <= ord(char) <= 0x9F
            for char in value
        ):
...
def _validate_options(value: OperationOptions) -> None:
    if (
        value.idempotency_key is not None
        and re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:-]{0,127}", value.idempotency_key)
        is None
    ):
...
def _profile_recovery_options(
    value: Optional[RecoveryOptions],
    maximum: int,
) -> RecoveryOptions:
...
def _read_profile_file(value: ProfileFile, maximum: int) -> bytes:
    ...
    if not _same_profile_file_snapshot(
