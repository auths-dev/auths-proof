...
const API = (query.get("api") ?? window.AUTHS_PAYMENT_COLLECT_API_BASE ?? "").replace(/\/$/, "");
...
function render(envelope) {
  ...
  const isDecision = kind === "merchant-collection-decision";
  ...
  elements.title.textContent =
    ...
    kind === "merchant-collection-observation" ? "Stripe was freshly observed." :
    "The collection lifecycle advanced durably.";
  ...
  elements.verification.dataset.state = state.includes("committed") || state === "eligible" ? "verified" : "denied";
  ...
  elements.profile.textContent = receipt.exact_action_profile ?? receipt.exact_action?.profile ?? "exact-payment-collect/1";
  elements.operation.textContent = receipt.operation ?? "collect";
