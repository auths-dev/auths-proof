...
const API = (window.AUTHS_PAYMENT_COLLECT_API_BASE ?? configuredApi).replace(/\/$/, "");
...
const elements = {
  ...
  reserved: $("#budget-reserved"), spent: $("#budget-spent"), unknown: $("#budget-unknown"),
};
...
function renderResult(result) {
  ...
  elements.outcome.dataset.kind =
    ["collected", "reconciled"].includes(result.outcome) ? "authorized" :
    result.outcome === "outcome-unknown" ? "indeterminate" : "denied";
  elements.detail.textContent = {
    collected: "Stripe accepted and a fresh retrieval proved one exact automatic-capture payment and Charge.",
    replay: "Durable state returned the original effect. No second PaymentIntent or Charge was requested.",
  ...
  }[result.outcome] ?? "The service failed closed at a typed collection lifecycle boundary.";
