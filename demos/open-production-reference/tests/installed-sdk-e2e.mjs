...
const endpoints = (process.env.AUTHS_REFERENCE_ENDPOINTS ?? "http://localhost:18081,http://localhost:18082,http://localhost:18083")
  .split(",");
...
for (const [name, profile] of profiles) {
  ...
  const issuer = createAuths({endpoint: endpoints[0], identity, profile});
  const authority = await issuer.create(decode(generated.request));
  assert.equal(authority.kind, "authority");
  const delegator = createAuths({endpoint: endpoints[1], identity, profile});
  ...
  const agentClient = createAuths({endpoint: endpoints[2], identity: agent, profile});
  const completed = await agentClient.execute(delegated, decode(generated.action));
  assert.equal(completed.kind, "completed");
  const verifier = createAuths({endpoint: endpoints[0], identity: agent, profile});
...
const issuer = createAuths({endpoint: endpoints[0], identity: recoveryIdentity, profile: recoveryProfile});
const delegator = createAuths({endpoint: endpoints[1], identity: recoveryIdentity, profile: recoveryProfile});
...
const agentClient = createAuths({endpoint: endpoints[2], identity: recoveryAgent, profile: recoveryProfile});
const unknown = await agentClient.execute(recoveryDelegated, decode(recovery.action));
assert.equal(unknown.kind, "recoverable");
const verifier = createAuths({endpoint: endpoints[0], identity: recoveryAgent, profile: recoveryProfile});
