...
test("all bounded-update experiments and the live result share one workbench", async () => {
  ...
  assert.match(html, /Complete receipt from the native receipt API/);
  assert.match(html, /There is no browser route that returns or delegates a connection string/);
});

test("frontend exercises readiness, exact execution, replay, rows, and receipts", async () => {
  ...
  assert.match(script, /result\.rows_after/);
  assert.match(script, /stateName === "replay"/);
  assert.match(script, /\/api\/v1\/receipts\//);
});

test("dedicated receipt route fails closed and renders native JSON", async () => {
...
test("browser security policy and native API proxy are explicit", async () => {
  const config = JSON.parse(await readFile(new URL("web/vercel.json", root), "utf8"));
