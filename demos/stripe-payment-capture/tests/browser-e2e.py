"""Browser E2E for a deployed bounded Stripe capture application.

Set AUTHS_STRIPE_PAYMENT_CAPTURE_E2E_URL to a real HTTP(S) deployment. The
...
base_url = os.environ.get("AUTHS_STRIPE_PAYMENT_CAPTURE_E2E_URL", "").rstrip("/")
if not base_url:
    raise SystemExit("AUTHS_STRIPE_PAYMENT_CAPTURE_E2E_URL is required")
...
def fresh(page):
    ...
    # new random, idempotent fixture; retry only session setup, never capture.
...
with sync_playwright() as playwright:
    ...
    fresh(page)
    run(page, "success", "CAPTURED")
    expect(page.locator("#durable-state")).to_have_text("capture-committed")
    expect(page.locator("#capturable-amount")).to_have_text("$0.00")
    expect(page.locator("#budget-held")).to_have_text("$0.00")
    expect(page.locator("#budget-spent")).to_have_text("$5.00")
    expect(page.locator("#receipt-json")).to_contain_text(
        "auths.stripe.payment-capture"
    )
    ...
    expect(receipt.locator("#profile")).to_contain_text("exact-payment-capture")
