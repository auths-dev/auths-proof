...
use auths_stripe::{
    DigestHex, MerchantCaptureReceipt, PortError, ReceiptSink,
...
impl ReceiptJournal {
    ...
    pub fn get(&self, receipt_id: &DigestHex) -> Result<Option<MerchantCaptureReceipt>, PortError> {
    ...
    pub fn read_all(&self) -> Result<Vec<MerchantCaptureReceipt>, PortError> {
        let _guard = self
            .process_lock
            .lock()
            .map_err(|_| PortError::Persistence)?;
...
impl ReceiptSink<MerchantCaptureReceipt> for ReceiptJournal {
    fn append(&self, receipt: &MerchantCaptureReceipt) -> Result<(), PortError> {
...
pub fn receipt_id(receipt: &MerchantCaptureReceipt) -> Result<DigestHex, PortError> {
    canonical_json(receipt)
        .map(|bytes| sha256(&bytes))
        .map_err(|_| PortError::Malformed)
}

fn read_all_unlocked(path: &Path) -> Result<Vec<MerchantCaptureReceipt>, PortError> {
...
fn read_all_from(file: &File) -> Result<Vec<MerchantCaptureReceipt>, PortError> {
    ...
    for line in BufReader::new(clone).split(b'\n') {
        ...
        let receipt: MerchantCaptureReceipt =
