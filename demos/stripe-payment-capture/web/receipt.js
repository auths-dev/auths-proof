...
const API = (query.get("api") ?? window.AUTHS_PAYMENT_CAPTURE_API_BASE ?? "").replace(/\/$/, "");
...
const elements = {
  ...
  policy: $("#policy-digest"), configuration: $("#configuration"), capture: $("#capture"),
...
function render(envelope) {
  ...
  const isDecision = kind === "merchant-capture-decision";
  ...
  elements.title.textContent =
    ...
    kind === "merchant-capture-observation" ? "Stripe was freshly observed." :
    "The capture lifecycle advanced durably.";
  elements.detail.textContent = `This is a ${kind} receipt. Capture, attempted execution, provider acceptance, and reconciled observation remain separately stated.`;
  elements.verification.dataset.state = state.includes("captured") || state === "eligible" ? "verified" : "denied";
  ...
  elements.profile.textContent = receipt.exact_action_profile ?? receipt.exact_action?.profile ?? "exact-payment-capture/1";
  elements.operation.textContent = receipt.operation ?? "capture";
  ...
  elements.capture.textContent = String(
    receipt.atomic_cross_budget_transition ??
    (receipt.bounded_decision?.code === "payment-capture-authorized")
  );
  ...
  pre.textContent = JSON.stringify({
    ...
    settlement_amount_minor: receipt.settlement_amount_minor,
    authorization_release_minor: receipt.authorization_release_minor,
    atomic_cross_budget_transition: receipt.atomic_cross_budget_transition,
    payment_intent_id: receipt.provider?.payment_intent_id,
    charge_id: receipt.provider?.charge_id,
    balance_transaction_id: receipt.provider?.balance_transaction_id,
  }, null, 2);
