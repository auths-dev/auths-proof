...
: Check that the ignored `.env` exists at
  `demos/stripe-payment-capture/.env`. Local development may assign one test
  key to both `AUTHS_STRIPE_FIXTURE_SECRET_KEY` and
  `AUTHS_STRIPE_PAYMENT_CAPTURE_SECRET_KEY`; production should use separate
  fixture and capture keys. Never paste either value into logs or issue
  reports.
...
```sh
rg -n 'client_secret|Capture: Bearer|sk_live_' \
  demos/stripe-payment-capture target
```
