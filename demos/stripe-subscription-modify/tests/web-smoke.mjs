...
test("copy exposes the atomic liability transition", async () => {
  const html = await readFile(new URL("web/index.html", root), "utf8");
  assert.match(html, /exact before state/i);
  assert.match(html, /credits never authorize/i);
  assert.match(html, /liability delta/i);
});
...
test("static deployment has restrictive policy and configurable API", async () => {
  ...
  assert.match(await readFile(new URL("web/config.js.example", root), "utf8"), /AUTHS_SUBSCRIPTION_MODIFY_API_BASE/);
});
