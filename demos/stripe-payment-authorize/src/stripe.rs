...
/// Fresh fixture returned before an exact authorization action is created.
...
impl LivePaymentAuthorizeEnvironment {
    ...
    pub fn from_environment() -> Result<Self, PortError> {
        Ok(Self {
            http: StripeHttp::from_environment("AUTHS_STRIPE_PAYMENT_AUTHORIZE_SECRET_KEY")?,
...
impl DemoPaymentAuthorizeEnvironment for LivePaymentAuthorizeEnvironment {
    fn seed_authorization(
        &self,
        workflow_id: &str,
        order_scope: &str,
        ...
        let customer = self.http.fixture_post(
            ...
            &[
                (
                    "description".into(),
                    "Auths bounded authorization demo".into(),
                ),
...
impl PaymentAuthorizeGateway for LivePaymentAuthorizeEnvironment {
    ...
    fn authorize(
        ...
        let response = match self.http.protected_post(
            ...
            Err(PortError::OutcomeUnknown) => {
                return Ok(PaymentAuthorizeEffect::OutcomeUnknown(None));
            }
        ...
        if ambiguous {
            return Ok(PaymentAuthorizeEffect::OutcomeUnknown(Some(provider)));
        }
        Ok(match provider.status.as_str() {
            "requires_capture" => PaymentAuthorizeEffect::Accepted(provider),
            "processing" => PaymentAuthorizeEffect::Processing(provider),
            "requires_action" => PaymentAuthorizeEffect::CustomerActionRequired(provider),
            "requires_payment_method" | "canceled" => PaymentAuthorizeEffect::Declined {
                code: "stripe-declined".into(),
            },
            _ => PaymentAuthorizeEffect::OutcomeUnknown(Some(provider)),
        })
    }
    ...
    fn reconcile(
        ...
        if let Some(provider) = record.provider() {
            ...
            return Ok(reconciled_authorization(observed));
        }
        let response = self.protected_get(
            &format!(
        ...
        let Some(found) = found else {
            return Ok(PaymentAuthorizeReconciliationOutcome::Released(None));
        };
        ...
        Ok(reconciled_authorization(observed))
    }
}

fn reconciled_authorization(
    provider: MerchantProviderProjection,
) -> PaymentAuthorizeReconciliationOutcome {
    match provider.status.as_str() {
        "requires_capture" => PaymentAuthorizeReconciliationOutcome::Held(provider),
        "requires_payment_method" | "canceled" => {
            PaymentAuthorizeReconciliationOutcome::Released(Some(provider))
        }
        _ => PaymentAuthorizeReconciliationOutcome::OutcomeUnknown(Some(provider)),
    }
}
