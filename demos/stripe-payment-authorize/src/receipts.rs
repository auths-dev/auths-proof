...
use auths_stripe::{
    DigestHex, MerchantAuthorizationReceipt, PortError, ReceiptSink,
...
impl ReceiptJournal {
    ...
    ) -> Result<Option<MerchantAuthorizationReceipt>, PortError> {
    ...
    pub fn read_all(&self) -> Result<Vec<MerchantAuthorizationReceipt>, PortError> {
        let _guard = self
            .process_lock
            .lock()
            .map_err(|_| PortError::Persistence)?;
...
impl ReceiptSink<MerchantAuthorizationReceipt> for ReceiptJournal {
    fn append(&self, receipt: &MerchantAuthorizationReceipt) -> Result<(), PortError> {
...
pub fn receipt_id(receipt: &MerchantAuthorizationReceipt) -> Result<DigestHex, PortError> {
    canonical_json(receipt)
        .map(|bytes| sha256(&bytes))
        .map_err(|_| PortError::Malformed)
}

fn read_all_unlocked(path: &Path) -> Result<Vec<MerchantAuthorizationReceipt>, PortError> {
...
fn read_all_from(file: &File) -> Result<Vec<MerchantAuthorizationReceipt>, PortError> {
    ...
    for line in BufReader::new(clone).split(b'\n') {
        ...
        let receipt: MerchantAuthorizationReceipt =
