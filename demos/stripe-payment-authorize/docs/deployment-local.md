...
```sh
docker compose \
  --env-file demos/stripe-payment-authorize/.env \
  -f demos/stripe-payment-authorize/compose.local.yaml \
  up --build
```
...
6. Verify the resulting PaymentIntent is `requires_capture`, the exact amount
   is capturable, the received amount is zero, and `capture_before` is present
   using the Stripe Dashboard or a credential-safe retrieval script.
...
```sh
cargo run -p auths-stripe-payment-authorize-demo
```
