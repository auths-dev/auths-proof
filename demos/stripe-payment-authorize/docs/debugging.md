...
: Check that the ignored `.env` exists at
  `demos/stripe-payment-authorize/.env`. Local development may use
  `AUTHS_STRIPE_TEST_SECRET_KEY`; production requires separate fixture and
  authorization keys. Never paste either value into logs or issue reports.
...
rg -n 'client_secret|Authorization: Bearer|sk_live_' \
  demos/stripe-payment-authorize target
```
