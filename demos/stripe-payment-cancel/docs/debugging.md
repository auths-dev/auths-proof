...
: Check that the ignored `.env` exists at
  `demos/stripe-payment-cancel/.env`. Local development may assign one test
  key to both `AUTHS_STRIPE_FIXTURE_SECRET_KEY` and
  `AUTHS_STRIPE_PAYMENT_CANCEL_SECRET_KEY`; production should use separate
  fixture and cancel keys. Never paste either value into logs or issue
  reports.

`stripe-fixture-unavailable`

: Session setup could not create the test Customer, attached PaymentMethod, or
  $10.00 manual-capture PaymentIntent. Confirm the fixture key has those setup
  permissions and that the pinned API version is enabled.
...
```sh
rg -n 'client_secret|Authorization: Bearer|sk_(test|live)_' \
  demos/stripe-payment-cancel target
```
