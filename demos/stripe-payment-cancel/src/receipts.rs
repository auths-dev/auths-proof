...
use auths_stripe::{
    DigestHex, MerchantCancelReceipt, PortError, ReceiptSink,
...
impl ReceiptJournal {
    ...
    pub fn get(&self, receipt_id: &DigestHex) -> Result<Option<MerchantCancelReceipt>, PortError> {
    ...
    pub fn read_all(&self) -> Result<Vec<MerchantCancelReceipt>, PortError> {
        let _guard = self
            .process_lock
            .lock()
            .map_err(|_| PortError::Persistence)?;
...
impl ReceiptSink<MerchantCancelReceipt> for ReceiptJournal {
    fn append(&self, receipt: &MerchantCancelReceipt) -> Result<(), PortError> {
...
pub fn receipt_id(receipt: &MerchantCancelReceipt) -> Result<DigestHex, PortError> {
    canonical_json(receipt)
        .map(|bytes| sha256(&bytes))
        .map_err(|_| PortError::Malformed)
}

fn read_all_unlocked(path: &Path) -> Result<Vec<MerchantCancelReceipt>, PortError> {
...
fn read_all_from(file: &File) -> Result<Vec<MerchantCancelReceipt>, PortError> {
    ...
    for line in BufReader::new(clone).split(b'\n') {
        ...
        let receipt: MerchantCancelReceipt =
